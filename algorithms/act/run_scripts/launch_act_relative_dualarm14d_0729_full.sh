...
DATA_ROOT="${DATA_ROOT:-/data/wengyikun/datasets/joint_songling/0729_doublefripper_top_grippebread_25fps}"
STATS_ROOT="$DATA_ROOT/normalization"
OUTPUT_DIR="${OUTPUT_DIR:-/data/wengyikun/outputs/act_0729_dualarm14d_relative_3cam_full_chunk16_b32_100k/train_out}"
STEPS="${STEPS:-100000}"
SAVE_FREQ="${SAVE_FREQ:-10000}"

# Keep the official ResNet18 backbone cache on the persistent data volume.
export TORCH_HOME="${TORCH_HOME:-/data/wengyikun/.cache/torch}"

exec accelerate launch --num_processes=1 --mixed_precision=bf16 \
  -m lerobot.scripts.lerobot_train \
  --dataset.repo_id=local/0729_doublefripper_top_grippebread_25fps_full \
  ...
  --output_dir="$OUTPUT_DIR" --job_name="${JOB_NAME:-act_0729_dualarm14d_relative_3cam_full_chunk16_b32_100k}"
