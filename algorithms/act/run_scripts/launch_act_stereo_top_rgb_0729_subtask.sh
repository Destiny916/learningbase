...
DATA_ROOT="${DATA_ROOT:-/data/wengyikun/datasets/joint_songling/0729_dualarm14d_stereo_top_rgbd_subtask_v30}"
STATS_ROOT="${STATS_ROOT:-$DATA_ROOT/normalization}"
OUTPUT_DIR="${OUTPUT_DIR:-/data/wengyikun/outputs/act_0729_stereo_top_rgb_chunk16_b32_500k/train_out}"
STEPS="${STEPS:-500000}"
...
exec accelerate launch --num_processes=1 --mixed_precision=bf16 \
  ...
  --dataset.repo_id=local/0729_dualarm14d_stereo_top_rgbd_subtask \
  --dataset.root="$DATA_ROOT" --dataset.depth_output_unit=m \
  --dataset.image_transforms.enable=false \
  --policy.type=act --policy.device=cuda --policy.push_to_hub=false \
  --policy.stereo_visual_mode=stereo_top_rgb \
  ...
  --batch_size="$BATCH_SIZE" --gradient_accumulation_steps="$GRAD_ACC" --num_workers=8 \
  --steps="$STEPS" --save_checkpoint=true --save_freq="$SAVE_FREQ" --log_freq=10 \
  --eval_steps=0 --wandb.enable=false \
  --output_dir="$OUTPUT_DIR" --job_name="${JOB_NAME:-act_0729_stereo_top_rgb_chunk16_b32_500k}"
