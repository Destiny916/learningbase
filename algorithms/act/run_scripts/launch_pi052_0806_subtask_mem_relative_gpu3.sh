...
DATA_ROOT="${DATA_ROOT:-/data/wengyikun/datasets/joint_songling/0806swap_gripper_fixed_pi052_task_en_subtask_mem}"
REL_ROOT="$DATA_ROOT/normalization_relative"
...
OUTPUT_DIR="${OUTPUT_DIR:-/data/wengyikun/outputs/pi052_0806_subtask_mem_relative_state_relative_action_chunk50_b1_400k_gpu3/train_out}"

for path in \
  "$DATA_ROOT/meta/info.json" \
  "$DATA_ROOT/subtask_memory_summary.json" \
  "$REL_ROOT/relative_stats_manifest.json" \
  "$REL_ROOT/relative_state_q01_q99.json" \
  "$REL_ROOT/relative_action_chunk50_q01_q99.json" \
...
exec accelerate launch --num_processes=1 --mixed_precision=bf16 \
  ...
  --dataset.repo_id=local/0806swap_gripper_fixed_pi052_task_en_subtask_mem \
  ...
  --policy.state_absolute_indices='[6,7,8,9,16,17,18,19]' \
  --policy.condition_on_state=true \
  --policy.relative_state_stats_path="$REL_ROOT/relative_state_q01_q99.json" \
  --policy.relative_action_stats_path="$REL_ROOT/relative_action_chunk50_q01_q99.json" \
  ...
  --job_name=pi052_0806_subtask_mem_relative_state_relative_action_chunk50_b1_400k_gpu3
