...
DATA_ROOT="${DATA_ROOT:-/data/wengyikun/datasets/joint_songling/0714_gripper_bread_normal_even_wrongplace_endpose10d_v30_split_seed42}"
...
JOB_NAME="${JOB_NAME:-pi05_0714_endpose10d_normal_even_wrongplace_absolute_chunk50}"
OUTPUT_DIR="${OUTPUT_DIR:-/data/wengyikun/outputs/pi05_0714_endpose10d_normal_even_wrongplace_absolute_chunk50/train_out}"
...
echo "absolute pose10d state/action quantiles: $STATS_ROOT"
...
exec accelerate launch "${ACCELERATE_ARGS[@]}" \
  ...
  --dataset.repo_id=local/0714_bread_endpose10d_absolute_train \
  --dataset.root="$TRAIN_ROOT" \
  --validation_dataset.repo_id=local/0714_bread_endpose10d_absolute_test \
