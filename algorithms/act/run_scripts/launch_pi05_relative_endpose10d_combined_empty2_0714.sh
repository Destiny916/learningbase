...
DATA_ROOT="${DATA_ROOT:-/data/wengyikun/datasets/joint_songling/0714_gripper_bread_single_teleop_normal_differentplace_wrongplace_right_fisheye_endpose_pose10d_combined_v30_joint7d_split_seed42}"
...
OUTPUT_DIR="${OUTPUT_DIR:-/data/wengyikun/outputs/pi05_0714_endpose10d_combined_relative_empty2/train_out}"
JOB_NAME="${JOB_NAME:-pi05_0714_endpose10d_combined_relative_empty2}"
...
exec accelerate launch --num_processes=1 --mixed_precision=bf16 \
  ...
  --dataset.repo_id=local/0714_bread_combined_endpose10d_train \
  --dataset.root="$TRAIN_ROOT" \
  --dataset.image_transforms.enable=false \
  --validation_dataset.repo_id=local/0714_bread_combined_endpose10d_test \
