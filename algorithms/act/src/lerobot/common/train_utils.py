...
def save_training_step(
    ...
    # These fields are recorded so a resumed run can detect a changed effective batch size: the
    # sampler's resume offset is computed from the values that produced `step`, since all of them
    # scale how many sampler positions an optimizer update consumes (see compute_sampler_state).
...
def save_checkpoint(
    ...
    Args:
        ...
        batch_size (int | None, optional): Per-process batch size to record for sample-exact
            resume. Defaults to None (not recorded).
        gradient_accumulation_steps (int | None, optional): Number of micro-batches consumed per
            optimizer update to record for sample-exact resume. Defaults to None (not recorded).
...
def save_training_state(
    ...
    Args:
        ...
        gradient_accumulation_steps (int | None, optional): Number of micro-batches consumed per
            optimizer update to record. Defaults to None.
