...
class TrainPipelineConfig(HubMixin):
    ...
    log_freq: int = 200
    # Compute eval loss on validation data every N steps (0 = disabled). Requires a validation source.
    eval_steps: int = 0
    ...
    def validate(self) -> None:
        ...
        for field_name in ("eval_steps", "max_eval_samples", "validation_seed"):
            value = getattr(self, field_name)
            if not isinstance(value, int) or isinstance(value, bool) or value < 0:
                raise ValueError(f"{field_name} must be a non-negative integer, got {value!r}.")

        if self.validation_dataset is not None:
            if self.dataset.eval_split > 0.0:
                raise ValueError("validation_dataset and dataset.eval_split > 0.0 are mutually exclusive.")
            if self.validation_dataset.eval_split != 0.0:
                raise ValueError(
                    "validation_dataset.eval_split must be 0.0 because an independent validation root "
                    "is used in full."
                )
            if self.validation_dataset.streaming:
                raise NotImplementedError("validation_dataset.streaming is not supported yet.")

        if self.eval_steps > 0 and self.validation_dataset is None and self.dataset.eval_split == 0.0:
            raise ValueError(
                "eval_steps > 0 requires validation_dataset or dataset.eval_split > 0.0."
            )
