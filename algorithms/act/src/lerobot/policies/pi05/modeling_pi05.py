...
def _assert_finite_for_debug(name: str, tensor: Tensor) -> None:
    """Fail at the first non-finite PI05 tensor when finite debugging is enabled."""
    if os.environ.get("LEROBOT_PI05_FINITE_DEBUG") != "1" or torch.isfinite(tensor).all():
        return
    raise FloatingPointError(f"PI05 non-finite tensor: {name}; shape={tuple(tensor.shape)} dtype={tensor.dtype}")


def _register_finite_grad_debug_hook(name: str, tensor: Tensor) -> None:
    """Check gradients at the image embedding boundary when finite debugging is enabled."""
    if os.environ.get("LEROBOT_PI05_FINITE_DEBUG") != "1" or not tensor.requires_grad:
        return

    def check_gradient(gradient: Tensor) -> Tensor:
        if not torch.isfinite(gradient).all():
            raise FloatingPointError(
                f"PI05 non-finite gradient: {name}; shape={tuple(gradient.shape)} dtype={gradient.dtype}"
            )
        return gradient

    tensor.register_hook(check_gradient)
    ...
    def embed_prefix(
        ...
        if self.checkpoint_vision_embeddings:
            ...
            for image_index, image in enumerate(images):
                _assert_finite_for_debug(f"image_{image_index}", image)
                img_emb = embed_image(image)
                _assert_finite_for_debug(f"image_embedding_{image_index}", img_emb)
                _register_finite_grad_debug_hook(f"image_embedding_{image_index}", img_emb)
...
class PI05Policy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        visual_pretrained_path = getattr(config, "visual_pretrained_path", None)
        if visual_pretrained_path:
            report = self.load_visual_pretrained_weights(
                visual_pretrained_path,
                include_projector=getattr(config, "visual_pretrained_include_projector", True),
            )
            logging.info("Loaded visual pretrained weights: %s", report)

        self.model.to(config.device)

        self.reset()

    def load_visual_pretrained_weights(
        self, checkpoint_path: str | Path, *, include_projector: bool = True
    ) -> dict[str, int]:
        """Load only compatible vision/projector tensors from a PI05/VISTA checkpoint."""
        ...
        if not checkpoint.exists():
            raise FileNotFoundError(f"Visual pretrained checkpoint not found: {checkpoint}")

        own_state = self.state_dict()
        selected: dict[str, Tensor] = {}
        skipped_unmatched = skipped_shape = selected_vision = selected_projector = 0
        with safe_open(checkpoint, framework="pt", device="cpu") as handle:
            for source_key in handle.keys():
                is_vision = "vision_tower" in source_key
                is_projector = "multi_modal_projector" in source_key
                if not is_vision and not (include_projector and is_projector):
                    continue
                candidates = [source_key, source_key.replace(".vision_tower.vision_model.", ".vision_tower.")]
                candidates += [key.removeprefix("model.") for key in candidates]
                candidates += [f"model.{key}" for key in candidates if not key.startswith("model.")]
                target_key = next((key for key in candidates if key in own_state), None)
                if target_key is None:
                    skipped_unmatched += 1
                    continue
                tensor = handle.get_tensor(source_key)
                if tuple(tensor.shape) != tuple(own_state[target_key].shape):
                    skipped_shape += 1
                    continue
                selected[target_key] = tensor.to(dtype=own_state[target_key].dtype)
                selected_vision += int(is_vision)
                selected_projector += int(is_projector)
        if not selected:
            raise RuntimeError(f"No compatible visual weights found in {checkpoint}")
        _, unexpected = self.load_state_dict(selected, strict=False)
        if unexpected:
            raise RuntimeError(f"Unexpected visual pretrained keys: {unexpected[:5]}")
        return {
    ...
    def _prepare_pretrained_state_dict(self, state_dict: dict[str, Tensor]) -> dict[str, Tensor]:
        runtime_keys = self.state_dict().keys()
        prepared_state_dict = {}
        for key, value in state_dict.items():
            ...
            prepared_state_dict[next((candidate for candidate in candidates if candidate in runtime_keys), key)] = value
    ...
    def forward(
        ...
        *,
        ...
        prefix_mask = _sample_training_rtc_prefix_mask(
            actions.shape[0],
            actions.shape[1],
            getattr(self.config, "rtc_training_max_delay", 0),
            actions.device,
        )
        ...
        original_action_dim = self.config.output_features[ACTION].shape[-1]
        losses = losses[:, :, :original_action_dim]

        action_is_pad = batch.get("action_is_pad")
        if action_is_pad is None:
            action_is_pad = torch.zeros(losses.shape[:2], dtype=torch.bool, device=losses.device)
        else:
            action_is_pad = action_is_pad.to(device=losses.device, dtype=torch.bool)
        if action_is_pad.shape != losses.shape[:2]:
            raise ValueError(
                f"action_is_pad must have shape {tuple(losses.shape[:2])}, got {tuple(action_is_pad.shape)}"
            )

        valid_steps = ~action_is_pad
        if prefix_mask is not None:
            prefix_mask = prefix_mask.to(device=losses.device, dtype=torch.bool)
            if prefix_mask.shape != losses.shape[:2]:
                raise ValueError(
                    f"RTC prefix mask must have shape {tuple(losses.shape[:2])}, "
                    f"got {tuple(prefix_mask.shape)}"
                )
            valid_steps &= ~prefix_mask

        valid_all = valid_steps.unsqueeze(-1).expand_as(losses)
        valid_values = valid_all.to(dtype=losses.dtype)
        loss_sum_per_sample = (losses * valid_values).sum(dim=(1, 2))
        loss_count_per_sample = valid_all.sum(dim=(1, 2))
        per_sample_loss = loss_sum_per_sample / loss_count_per_sample.clamp_min(1)
        loss = loss_sum_per_sample.sum() / loss_count_per_sample.sum().clamp_min(1)

        loss_sum_per_dim = (losses * valid_values).sum(dim=(0, 1))
        loss_count_per_dim = valid_all.sum(dim=(0, 1)).clamp_min(1)

        gripper_indices = self.config.joint_gripper_indices
        if (
            not gripper_indices
            ...
            or any(
                ...
                or index < 0
                or index >= original_action_dim
                for index in gripper_indices
            )
        ):
            raise ValueError(
                f"gripper indices must be unique integers within [0, {original_action_dim}), "
                f"got {gripper_indices}"
            )

        gripper_losses = losses[:, :, gripper_indices]
        valid_grippers = valid_steps.unsqueeze(-1).expand_as(gripper_losses)
        gripper_values = valid_grippers.to(dtype=losses.dtype)
        gripper_loss_sum_per_sample = (gripper_losses * gripper_values).sum(dim=(1, 2))
        gripper_loss_count_per_sample = valid_grippers.sum(dim=(1, 2))
        gripper_loss_per_sample = (
            gripper_loss_sum_per_sample / gripper_loss_count_per_sample.clamp_min(1)
        )
        gripper_loss = (
            gripper_loss_sum_per_sample.sum() / gripper_loss_count_per_sample.sum().clamp_min(1)
        )

        loss_dict = {
            "loss": loss.detach().item(),
            "loss_per_dim": (loss_sum_per_dim / loss_count_per_dim).detach().cpu().tolist(),
            "loss_sum_per_sample": loss_sum_per_sample.detach(),
            "loss_count_per_sample": loss_count_per_sample.detach(),
            "gripper_loss": gripper_loss.detach().item(),
            "gripper_loss_sum_per_sample": gripper_loss_sum_per_sample.detach(),
            "gripper_loss_count_per_sample": gripper_loss_count_per_sample.detach(),
            "gripper_loss_per_sample": gripper_loss_per_sample.detach(),
        }
