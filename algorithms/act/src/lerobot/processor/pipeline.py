...
class DataProcessorPipeline(HubMixin, Generic[TInput, TOutput]):
    ...
    def train(self) -> None:
        """Enable train-only behavior on processor steps that implement it."""
        for processor_step in self.steps:
            train = getattr(processor_step, "train", None)
            if callable(train):
                train()

    def eval(self) -> None:
        """Disable train-only behavior on processor steps that implement it."""
        for processor_step in self.steps:
            eval_fn = getattr(processor_step, "eval", None)
            if callable(eval_fn):
                eval_fn()
