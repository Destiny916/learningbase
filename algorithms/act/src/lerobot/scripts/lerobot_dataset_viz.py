...
def action_series_for_visualization(action: torch.Tensor | np.ndarray) -> list[tuple[str, np.ndarray]]:
    """Convert a flat action or action chunk into Rerun scalar series."""
    values = action.detach().cpu().numpy() if isinstance(action, torch.Tensor) else np.asarray(action)
    if values.ndim == 1:
        return [(ACTION, values)]
    if values.ndim == 2:
        return [(f"{ACTION}/chunk/{step:02d}", values[step]) for step in range(values.shape[0])]
    raise ValueError(f"Expected action shape [A] or [H, A], got {tuple(values.shape)}")
...
def build_blueprint_from_dataset(dataset: LeRobotDataset):
    ...
    if action_feature and len(action_feature["shape"]) == 2:
        action_names = get_feature_names(dataset, ACTION)
        action_paths = [f"{ACTION}/chunk/{step:02d}" for step in range(action_feature["shape"][0])]
        action_overrides = {path: rr.SeriesLines(names=action_names) for path in action_paths}
        views.append(rrb.TimeSeriesView(name=ACTION, contents=action_paths, overrides=action_overrides))
    elif ACTION in dataset.features:
        action_names = get_feature_names(dataset, ACTION)
        styling = rr.SeriesLines(names=action_names)
        views.append(rrb.TimeSeriesView(origin=ACTION, name=ACTION, overrides={ACTION: styling}))

    if OBS_STATE in dataset.features:
        origin, key = "state", OBS_STATE
    ...
    **kwargs,
    ...
    for batch in tqdm.tqdm(dataloader, total=len(dataloader)):
        ...
        for i in range(len(batch["index"])):
            ...
            if ACTION in batch:
                for action_path, action_values in action_series_for_visualization(batch[ACTION][i]):
                    rr.log(action_path, rr.Scalars(action_values))
