...
from lerobot.configs import FeatureType, JobConfig, NormalizationMode, parser
...
def _format_debug_batch_metadata(batch: dict[str, Any]) -> dict[str, list[Any]]:
    """Return dataset identifiers that make a bad training batch reproducible."""
    metadata = {}
    for key in ("index", "episode_index", "frame_index"):
        value = batch.get(key)
        if isinstance(value, torch.Tensor):
            metadata[key] = value.detach().cpu().reshape(-1).tolist()
    return metadata


def _raise_if_nonfinite_gradients(policy: torch.nn.Module, batch: dict[str, Any]) -> None:
    """Fail fast with the offending parameter and dataset rows in opt-in debug mode."""
    if os.environ.get("LEROBOT_PI05_FINITE_DEBUG") != "1":
        return

    for name, parameter in policy.named_parameters():
        if parameter.grad is not None and not torch.isfinite(parameter.grad).all():
            raise FloatingPointError(
                "Non-finite gradient after backward: "
                f"parameter={name}; batch_metadata={_format_debug_batch_metadata(batch)}"
            )


def format_named_metrics(step: int, metrics: dict[str, float]) -> str:
    """Format stable train/validation metric keys for console logs and smoke-test parsing."""
    ordered = [key for key in _NAMED_METRIC_ORDER if key in metrics]
    extras = sorted(set(metrics) - set(_NAMED_METRIC_ORDER))
    values = " ".join(f"{key}={metrics[key]:.6g}" for key in [*ordered, *extras])
    return f"step={step} {values}"


def _normalization_processor_overrides(
    *,
    resume: bool,
    stats: dict[str, Any],
    ...
    """Keep checkpoint normalization statistics authoritative when resuming."""
    if resume:
        return {}, {}
...
def audit_pi05_parameters(policy: torch.nn.Module) -> dict[str, dict[str, int]]:
    """Count total and trainable PI0.5 parameters by runtime ownership group."""
    audit = {
        "language_model": {"total": 0, "trainable": 0},
        "vision_encoder": {"total": 0, "trainable": 0},
        "multimodal_projector": {"total": 0, "trainable": 0},
        "action_expert_other": {"total": 0, "trainable": 0},
    }
    for name, parameter in policy.named_parameters():
        if "language_model" in name:
            group = "language_model"
        elif "vision_tower" in name or "vision_encoder" in name:
            group = "vision_encoder"
        elif "multi_modal_projector" in name or "multimodal_projector" in name:
            group = "multimodal_projector"
        else:
            group = "action_expert_other"
        count = parameter.numel()
        audit[group]["total"] += count
        if parameter.requires_grad:
            audit[group]["trainable"] += count
    return audit


def log_pi05_parameter_audit(policy: torch.nn.Module) -> None:
    for group, counts in audit_pi05_parameters(policy).items():
        logging.info(
            "PI05 parameter audit %s: total=%s trainable=%s",
            group,
            format_big_number(counts["total"]),
            format_big_number(counts["trainable"]),
        )
...
def update_policy(
...
) -> tuple[MetricsTracker, dict | None, dict[str, float] | None]:
    """
    ...
    Returns:
        A tuple containing:
        - The unchanged MetricsTracker; the caller records metrics once per optimizer update.
        - A dictionary of outputs from the policy's forward pass, for logging purposes.
        - A dictionary of scalar metrics when an optimizer update was performed, otherwise None.
    """
    policy.train()
    ...
    _raise_if_nonfinite_gradients(policy, batch)
    ...
    if accelerator.sync_gradients:
        ...
        if torch.cuda.is_available():
            update_metrics["gpu_mem_gb"] = torch.cuda.max_memory_allocated() / (1024**3)

    return train_metrics, output_dict, update_metrics
...
def train(cfg: TrainPipelineConfig, accelerator: "Accelerator | None" = None):
    ...
    if cfg.is_reward_model_training:
        ...
        if is_main_process and cfg.policy.type == "pi05":
            log_pi05_parameter_audit(policy)
    ...
    processor_norm_map = active_cfg.normalization_mapping
    if getattr(active_cfg, "type", None) == "pi05":
        ...
        processor_dataset_stats = merge_pi05_joint_stats(active_cfg, dataset.meta.stats)
        processor_norm_map = dict(active_cfg.normalization_mapping)
        if can_use_pi05_joint_stats(active_cfg):
            processor_norm_map[FeatureType.STATE] = NormalizationMode.MIN_MAX
            processor_norm_map[FeatureType.ACTION] = NormalizationMode.MIN_MAX
    ...
    processor_kwargs.update(
        {
            ...
            "dataset_exclude_episodes": getattr(cfg.dataset, "exclude_episodes", None),
        }
    )
    ...
    if not cfg.is_reward_model_training and processor_pretrained_path is not None:
        ...
        normalization_preprocessor_overrides, normalization_postprocessor_overrides = (
            _normalization_processor_overrides(
                resume=cfg.resume,
                stats=processor_dataset_stats,
                input_features=policy.config.input_features,
                output_features=policy.config.output_features,
                norm_map=processor_norm_map,
            )
        )
        preprocessor_overrides.update(normalization_preprocessor_overrides)
        postprocessor_overrides.update(normalization_postprocessor_overrides)
        if getattr(active_cfg, "type", None) == "pi05":
            preprocessor_overrides["pi05_joint_representation_processor"] = {
                "joint_representation": getattr(active_cfg, "joint_representation", "absolute"),
                "gripper_indices": getattr(active_cfg, "joint_gripper_indices", [6, 13]),
                "action_names": getattr(active_cfg, "action_feature_names", None),
                "joint_limit_profile": getattr(active_cfg, "joint_limit_profile", "piper_pika_14d"),
            }
            postprocessor_overrides["pi05_absolute_action_processor"] = {
                "joint_representation": getattr(active_cfg, "joint_representation", "absolute"),
                "gripper_indices": getattr(active_cfg, "joint_gripper_indices", [6, 13]),
                "joint_limit_profile": getattr(active_cfg, "joint_limit_profile", "piper_pika_14d"),
            }
        if getattr(active_cfg, "use_relative_actions", False) and getattr(
            active_cfg, "joint_representation", "absolute"
        ) == "absolute":
    ...
    if not cfg.dataset.streaming:
        ...
        if cfg.resume and step > 0:
            ...
            ckpt_gradient_accumulation_steps = (
                saved_gradient_accumulation_steps or cfg.gradient_accumulation_steps
            )
            ...
            if is_main_process and saved_gradient_accumulation_steps not in (
                None,
                cfg.gradient_accumulation_steps,
            ):
                logging.warning(
                    "Resuming with gradient_accumulation_steps="
                    f"{cfg.gradient_accumulation_steps} but the checkpoint was written with "
                    f"gradient_accumulation_steps={saved_gradient_accumulation_steps}. The data order "
            ...
            sampler_state = compute_sampler_state(
                step,
                len(sampler),
                ckpt_batch_size * ckpt_gradient_accumulation_steps,
                ckpt_num_processes,
            )
    ...
    track_gripper_loss = getattr(active_cfg, "type", None) in {"pi05", "act"}
    if track_gripper_loss:
        train_metrics["gripper_loss"] = AverageMeter("grip", ":.3f")
    if torch.cuda.is_available():
    ...
    for _ in range(step, cfg.steps):
        ...
        output_dict = None
        update_metrics = None
        train_gripper_parts = SampleMetricAccumulator("gripper_loss") if track_gripper_loss else None
        ...
        for _micro_step in range(cfg.gradient_accumulation_steps):
            ...
            with accelerator.accumulate(policy):
                train_tracker, output_dict, update_metrics = update_policy(
                    train_tracker,
                    policy,
                    batch,
                    optimizer,
                ...
                if train_gripper_parts is not None:
                    if output_dict is None:
                        raise RuntimeError("PI0.5/ACT forward did not return gripper loss details")
                    train_gripper_parts.update(output_dict)

        if update_metrics is None:
            raise RuntimeError(
                "Gradient accumulation finished without an optimizer update. "
                "Check Accelerate gradient_accumulation_steps configuration."
            )

        train_tracker.dataloading_s = dataloading_s
        train_tracker.update_s = time.perf_counter() - update_start_time - dataloading_s
        train_tracker.loss = update_metrics["loss"]
        train_tracker.grad_norm = update_metrics["grad_norm"]
        train_tracker.lr = update_metrics["lr"]
        named_train_metrics = {"train/loss": update_metrics["loss"]}
        if train_gripper_parts is not None:
            train_gripper_loss = train_gripper_parts.compute_global(accelerator)
            train_tracker.gripper_loss = train_gripper_loss
            named_train_metrics["train/gripper_loss"] = train_gripper_loss
        if torch.cuda.is_available() and "gpu_mem_gb" in update_metrics:
            train_tracker.gpu_mem_gb = update_metrics["gpu_mem_gb"]
        ...
        if is_log_step:
            ...
            if is_main_process:
                logging.info(format_named_metrics(step, named_train_metrics))
                ...
                if wandb_logger:
                    ...
                    if output_dict:
                        wandb_log_dict.update(
                            {key: value for key, value in output_dict.items() if key != "gripper_loss"}
                        )
        ...
        if is_eval_step:
            preprocessor.eval()
            try:
                validation_metrics = evaluate_offline(
                    policy,
                    eval_dataloader,
                    preprocessor,
                    accelerator,
                    ...
                    camera_keys=eval_dataset.meta.camera_keys,
                )
            finally:
                preprocessor.train()

            if is_main_process:
                logging.info(format_named_metrics(step, {**named_train_metrics, **validation_metrics}))
                if wandb_logger:
                    wandb_logger.log_dict(
                        {key.removeprefix("valid/"): value for key, value in validation_metrics.items()},
                        step=step,
                        mode="valid",
                    )
