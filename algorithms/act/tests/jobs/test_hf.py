...
def test_pod_forwarded_args_drops_host_only_flags():
    ...
    argv = [
        ...
        "--dataset.root",
        "/other/local/data",
        "--validation_dataset.root=/local/validation",
...
def test_build_remote_config_applies_overrides(tmp_path):
    cfg = _minimal_cfg()
    cfg.validation_dataset = type(cfg.dataset)(repo_id="u/validation", root="/local/validation")
    cfg.validation_seed = 17
    ...
    assert data["validation_dataset"]["root"] is None
    assert data["validation_seed"] == 17
    ...
    assert cfg.validation_dataset.root == "/local/validation"


@pytest.mark.parametrize("eval_split", [0.0, 0.2], ids=["no-eval", "internal-eval"])
def test_build_remote_config_omits_external_validation_fields_when_unused(tmp_path, eval_split):
    cfg = _minimal_cfg()
    cfg.dataset.eval_split = eval_split
    cfg.validation_seed = 17
    dest = tmp_path / "train_config.json"
    ...
    assert "validation_seed" not in data


def test_resume_job_drops_validation_dataset_root(monkeypatch):
    cfg = _minimal_cfg()
    monkeypatch.setattr(
        "sys.argv",
        [
            "lerobot-train",
            "--config_path=user/checkpoints",
            "--validation_dataset.root=/local/validation",
            "--steps=10",
        ],
    )
    monkeypatch.setattr("lerobot.jobs.hf.parser.parse_arg", lambda name: "user/checkpoints")

    _, command = _build_resume_job(cfg, "alice")

    assert "--validation_dataset.root=/local/validation" not in command
...
def test_submit_passes_validation_and_submits(monkeypatch):
    ...
    dataset_calls = []
    monkeypatch.setattr(
        "lerobot.jobs.hf.ensure_dataset_available",
        lambda repo_id, **kwargs: dataset_calls.append((repo_id, kwargs["root"])),
    )
    ...
    cfg.dataset.root = "/local/train"
    cfg.validation_dataset.root = "/local/validation"
    ...
    assert dataset_calls == [("u/d", "/local/train"), ("u/validation", "/local/validation")]


@pytest.mark.parametrize(
    "validation_root",
    ["/local/train", "/local/validation-for-same-repo"],
    ids=["same-root", "different-root"],
)
def test_submit_rejects_same_repo_for_external_validation(monkeypatch, validation_root):
    monkeypatch.setattr("lerobot.jobs.hf.get_token", lambda: "tok")
    ...
    run_job_mock = MagicMock(return_value=SimpleNamespace(id="job-1", url=None))
    monkeypatch.setattr("lerobot.jobs.hf.run_job", run_job_mock)
    cfg = draccus.parse(
        TrainPipelineConfig,
        args=[
            "--dataset.repo_id",
    ...
    cfg.dataset.root = "/local/train"
    cfg.validation_dataset.root = validation_root

    with pytest.raises(
        ValueError, match="independent validation roots require distinct Hub repo_ids"
    ):
        submit_to_hf(cfg)
