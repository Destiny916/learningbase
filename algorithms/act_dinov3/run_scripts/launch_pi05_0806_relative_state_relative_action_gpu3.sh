...
REL_ROOT="${REL_ROOT:-$DATA_ROOT/normalization_relative_pi05_action_aligned_v2}"
PI05_BASE="${PI05_BASE:-/data/wengyikun/openpi/lerobot_pi05_base}"
PALIGEMMA_TOKENIZER="${PALIGEMMA_TOKENIZER:-/data/jianan/weight/paligemma-3b-pt-224}"
BATCH_SIZE="${BATCH_SIZE:-16}"
STEPS="${STEPS:-400000}"
SAVE_FREQ="${SAVE_FREQ:-40000}"
OUTPUT_DIR="${OUTPUT_DIR:-/data/wengyikun/outputs/pi05_0806_relative_state_relative_action_task_en_chunk50_b16_400k_gpu3/train_out}"
JOB_NAME="${JOB_NAME:-pi05_0806_relative_state_relative_action_task_en_chunk50_b16_400k_gpu3}"
...
for path in \
  ...
  "$PI05_BASE/config.json" \
  "$PI05_BASE/model.safetensors"; do
...
exec accelerate launch --num_processes=1 --mixed_precision=bf16 \
  ...
  --policy.state_position_noise_std_m=0 \
