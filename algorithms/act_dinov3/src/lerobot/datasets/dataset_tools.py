...
def _read_data_parquet_as_dataframe(path: Path) -> pd.DataFrame:
    """Read data parquet without pandas extension dtypes for fixed-rank arrays."""
    table = pq.read_table(path)
    return pd.DataFrame({column_name: table[column_name].to_pylist() for column_name in table.column_names})
...
def _copy_and_reindex_data(
    ...
    if dst_meta.tasks is None:
        ...
        for src_path in file_to_episodes:
            df = _read_data_parquet_as_dataframe(src_dataset.root / src_path)
