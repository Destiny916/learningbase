...
def build_remote_config_file(cfg, repo_id: str, dest: Path, tags: list[str] | None = None) -> Path:
    ...
    compatible with the released `lerobot-gpu` image. Configs using
    `validation_dataset` retain that field and require a current code image.
    `tags` are merged into policy.tags so the trained model the pod pushes carries them too.
    """
    ...
    if remote.validation_dataset is not None:
        # Keep the new field in the staged config; external validation requires a current code image.
        remote.validation_dataset.root = None
    if tags:
    ...
    if remote.validation_dataset is None:
        data.pop("validation_dataset", None)
        data.pop("validation_seed", None)
...
def submit_to_hf(cfg: TrainPipelineConfig) -> None:
    ...
    if (
        cfg.validation_dataset is not None
        and cfg.validation_dataset.repo_id == cfg.dataset.repo_id
    ):
        raise ValueError(
            "External validation uses an independent dataset root; independent validation roots "
            "require distinct Hub repo_ids. "
            f"train repo_id={cfg.dataset.repo_id!r}, root={cfg.dataset.root!r}; "
            f"validation repo_id={cfg.validation_dataset.repo_id!r}, "
            f"root={cfg.validation_dataset.root!r}."
        )
    ...
    datasets_to_publish = [(cfg.dataset.repo_id, cfg.dataset.root)]
    if (
        cfg.validation_dataset is not None
        and cfg.validation_dataset.repo_id not in {repo_id for repo_id, _ in datasets_to_publish}
    ):
        datasets_to_publish.append(
            (cfg.validation_dataset.repo_id, cfg.validation_dataset.root)
        )
    for dataset_repo_id, dataset_root in datasets_to_publish:
        ensure_dataset_available(dataset_repo_id, api=api, root=dataset_root, tags=tags)
