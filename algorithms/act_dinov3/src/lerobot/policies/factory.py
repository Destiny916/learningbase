...
from .act_dinov3.configuration_act_dinov3 import ACTDINOv3Config
...
from .pi05.joint_representation import Pi05AbsoluteActionProcessorStep, Pi05JointRepresentationProcessorStep
...
def _reconnect_relative_absolute_steps(
    ...
    """Reconnect relative-action processor pairs after deserialization.

    After a policy is loaded from disk, the preprocessor and postprocessor are reconstructed
    independently from their configs. Relative absolute-action steps need a live reference
    to their matching preprocessor step so they can read inference-time cached state.
    Those references are not serializable, so we re-establish them here after loading.
    """
    ...
    relative_joint_step = next(
        (s for s in preprocessor.steps if isinstance(s, RelativeJointProcessorStep)), None
    )
    if relative_joint_step is not None:
        for step in postprocessor.steps:
            if isinstance(step, RelativeJointAbsoluteActionProcessorStep) and step.relative_step is None:
                step.relative_step = relative_joint_step

    pi05_joint_step = next(
        (s for s in preprocessor.steps if isinstance(s, Pi05JointRepresentationProcessorStep)), None
    )
    if pi05_joint_step is not None:
        for step in postprocessor.steps:
            if isinstance(step, Pi05AbsoluteActionProcessorStep) and step.relative_step is None:
                step.relative_step = pi05_joint_step
...
def get_policy_class(name: str) -> type[PreTrainedPolicy]:
    ...
    if name == "tdmpc":
        ...
        return ACTPolicy
    elif name == "act_dinov3":
        from .act_dinov3.modeling_act_dinov3 import ACTDINOv3Policy

        return ACTDINOv3Policy
...
def make_policy_config(policy_type: str, **kwargs) -> PreTrainedConfig:
    ...
    elif policy_type == "act_dinov3":
        return ACTDINOv3Config(**kwargs)
...
def make_pre_post_processors(
    ...
    **kwargs: Unpack[ProcessorConfigKwargs],
    ...
    if pretrained_path:
        ...
        if policy_cfg.type == "pi052" and kwargs.get("dataset_stats") is not None:
            from .pi052.processor_pi052 import make_pi052_pre_post_processors

            return make_pi052_pre_post_processors(
                config=policy_cfg,
        ...
        # Relative-joint checkpoints do not include the generic rename step. An empty
        # rename map is a no-op, so do not send it through strict override validation.
        if preprocessor_overrides.get("rename_observations_processor") == {"rename_map": {}}:
            preprocessor_overrides.pop("rename_observations_processor")
    ...
    elif isinstance(policy_cfg, ACTDINOv3Config):
