    ...
    for key, ft in features.items():
        ...
        if ft["dtype"] in ["image", "video"]:
            ...
            if len(shape) != 3:
                ...
                names = ft["names"]
                if names is None:
                    first_is_channel = shape[0] in (1, 3, 4)
                    last_is_channel = shape[2] in (1, 3, 4)
                    if first_is_channel == last_is_channel:
                        raise ValueError(
                            f"Visual feature {key} has names=None and shape={shape}; channel layout is ambiguous "
                            "because exactly one of the first or last dimensions must be 1, 3, or 4."
                        )
                    if last_is_channel:
                ...
                elif names[2] in ["channel", "channels"]:  # (h, w, c) -> (c, h, w)
                    shape = (shape[2], shape[0], shape[1])
