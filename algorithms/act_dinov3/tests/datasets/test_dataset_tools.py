...
def test_split_preserves_two_dimensional_action_features(tmp_path, empty_lerobot_dataset_factory):
    dataset = empty_lerobot_dataset_factory(
        root=tmp_path / "source",
        features={
            "action": {"dtype": "float32", "shape": (2, 3), "names": None},
    ...
    for episode_index in range(2):
        for frame_index in range(2):
            dataset.add_frame(
                {
                    "action": np.full((2, 3), episode_index * 10 + frame_index, dtype=np.float32),
                    "observation.state": np.array([episode_index, frame_index], dtype=np.float32),
                    "task": "test task",
                }
            )
        dataset.save_episode()
    ...
    assert tuple(splits["train"][0]["action"].shape) == (2, 3)
    assert tuple(splits["test"][0]["action"].shape) == (2, 3)
    torch.testing.assert_close(splits["train"][0]["action"], torch.zeros(2, 3))
    torch.testing.assert_close(splits["test"][0]["action"], torch.full((2, 3), 10.0))
