...
def test_quantile_normalization_clips_when_enabled():
    ...
    transition = create_transition(observation={OBS_STATE: torch.tensor([-5.0, 2.0])})

    normalized = normalizer(transition)[TransitionKey.OBSERVATION][OBS_STATE]

    assert torch.equal(normalized, torch.tensor([-1.0, 1.0]))


def test_quantile_unnormalization_does_not_clip_when_enabled():
    features = {ACTION: PolicyFeature(FeatureType.ACTION, (2,))}
    norm_map = {FeatureType.ACTION: NormalizationMode.QUANTILES}
    stats = {ACTION: {"q01": [0.0, -1.0], "q99": [10.0, 1.0]}}
    ...
    transition = create_transition(action=torch.tensor([-2.0, 2.0]))

    unnormalized = unnormalizer(transition)[TransitionKey.ACTION]

    assert torch.equal(unnormalized, torch.tensor([-5.0, 2.0]))
