...
if [[ "${IN_TURBOVLA_CONTAINER:-0}" != "1" ]]; then
  ...
  exec docker run --rm --gpus "device=${selected_gpu}" --ipc=host \
    ...
    -e DINOV3_MODEL_PATH \
    -e RUN_ROOT_DIR \
    -e RUN_ID="${RUN_ID:-turbovla_joint_songling_730_endpoint20_3view}" \
    -e JOINT_SONGLING_OVERLAY_ROOT="${JOINT_SONGLING_OVERLAY_ROOT:-}" \
    -v "${repo_root}:${repo_root}" \
    -v /data:/data \
    -w "$repo_root" \
    "$image" bash scripts/joint_songling/train_endpoint20_3view.sh "$@"
fi
...
export JOINT_SONGLING_OVERLAY_ROOT="${JOINT_SONGLING_OVERLAY_ROOT:-${RUN_ROOT_DIR}/joint_songling_730_endpoint20_overlay}"
...
for name in ("info.json", "episodes", "stats.json", "tasks.parquet"):
    target = meta / name
    if not target.exists():
...
modality = {
    "state": {
        ...
        "right_endpoint": {"start": 14, "end": 17, "original_key": "observation.state"},
        "left_endpoint": {"start": 17, "end": 20, "original_key": "observation.state"},
    },
    "action": {
    ...
    "video": {
        "top": {"original_key": "observation.images.top_right"},
...
print("[INFO] camera_order=top_right,gripper_left,gripper_right")
print("[INFO] state_dim=20 (endpoint xyz absolute), action_dim=14")
PY
...
exec python3 -m accelerate.commands.launch --num_processes 1 \
  ...
  --config_yaml experiments/joint_songling/configs/endpoint20_3view.yaml \
  --run_root_dir "$RUN_ROOT_DIR" \
  --run_id "${RUN_ID:-turbovla_joint_songling_730_endpoint20_3view}" \
  "$@"
