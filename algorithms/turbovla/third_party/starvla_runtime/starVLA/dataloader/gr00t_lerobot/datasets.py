...
def _video_resolution_from_feature(feature: dict) -> tuple[int, int, int]:
    """Return ``(width, height, channels)`` from LeRobot v2/v3 video metadata."""
    shape = feature["shape"]
    ...
    if len(shape) != 3:
        raise ValueError(f"Expected unnamed video shape (H, W, C), got {shape}")
    height, width, channels = shape
    return width, height, channels


def preprocess_joint_songling_frame(image: np.ndarray, view_index: int) -> np.ndarray:
    """Apply Joint Songling camera geometry before the common 224px resize."""
    if image.ndim != 3 or image.shape[-1] != 3:
        raise ValueError(f"Expected an HWC RGB image, got {image.shape}")
    if view_index == 0:
        if image.shape != (405, 720, 3):
            raise ValueError(f"joint_songling top view must be 405x720x3, got {image.shape}")
        return np.pad(image, ((157, 158), (0, 0), (0, 0)), mode="constant")
    if view_index in (1, 2):
        if image.shape != (480, 640, 3):
            raise ValueError(f"joint_songling wrist view must be 480x640x3, got {image.shape}")
        return image[:, 80:560, :]
    raise ValueError(f"joint_songling expects view index 0, 1, or 2, got {view_index}")


def preprocess_joint_songling_top_padded_frame(image: np.ndarray, view_index: int) -> np.ndarray:
    """Pad only the 405x720 top view; keep both 480x640 wrist views uncropped."""
    if image.ndim != 3 or image.shape[-1] != 3:
        raise ValueError(f"Expected an HWC RGB image, got {image.shape}")
    if view_index == 0:
        if image.shape != (405, 720, 3):
            raise ValueError(
                f"joint_songling_top_padded top view must be 405x720x3, got {image.shape}"
            )
        return np.pad(image, ((157, 158), (0, 0), (0, 0)), mode="constant")
    if view_index in (1, 2):
        if image.shape != (480, 640, 3):
            raise ValueError(
                f"joint_songling_top_padded wrist view must be 480x640x3, got {image.shape}"
            )
        return image
    raise ValueError(
        f"joint_songling_top_padded expects view index 0, 1, or 2, got {view_index}"
    )
...
def _normalize_state_mode(mode: str) -> str:
    """Normalize state mode names to {abs, delta}."""
    mode = str(mode).lower()
    if mode in {"absolute", "raw"}:
        mode = "abs"
...
def _normalize_state_mode_apply_keys(
    state_mode_apply_keys: Sequence[str] | None,
    fallback_keys: Sequence[str] | None = None,
) -> list[str]:
    source_keys = state_mode_apply_keys if state_mode_apply_keys else (fallback_keys or [])
    normalized = []
    for key in source_keys:
        key = str(key)
...
def relative_arm_state_from_history(state_history: np.ndarray, arm_indices: np.ndarray) -> np.ndarray:
    """Return the current state with selected arm dimensions expressed as q[t]-q[t-1]."""
    if state_history.ndim != 2 or len(state_history) < 2:
        raise ValueError(f"Expected at least two state frames [T, D], got {state_history.shape}")
    output = state_history[-1:].copy()
    output[:, arm_indices] -= state_history[-2:-1, arm_indices]
    return output
...
def _infer_lerobot_version(dataset_path: Path, data_cfg=None) -> str:
    """Infer the LeRobot dataset version from config, metadata, or file layout."""
    if data_cfg is not None:
        configured = data_cfg.get("lerobot_version", None)
        if configured:
            return str(configured)

    info_meta_path = dataset_path / LE_ROBOT_INFO_FILENAME
    if info_meta_path.exists():
        try:
            with open(info_meta_path, "r") as f:
                info_meta = json.load(f)
            codebase_version = str(info_meta.get("codebase_version", "")).strip().lower()
...
def _build_stats_cache_config(
    action_mode: str,
    action_mode_apply_keys: list[str],
    state_mode: str,
    state_mode_apply_keys: list[str],
) -> dict:
    return {
        "action_mode": action_mode,
        "action_mode_apply_keys": action_mode_apply_keys,
        "relative_action_anchor": "current" if action_mode == "rel" else None,
        "state_mode": state_mode,
        "state_mode_apply_keys": state_mode_apply_keys,
    }
...
def _compute_statistics_for_mode(
    ...
    state_mode: str,
    state_mode_apply_keys: list[str] | None,
) -> dict:
    ...
    if state_mode == "delta":
        base_stats = calculate_delta_state_statistics(
...
def calculate_delta_state_statistics(
    parquet_paths: list[Path],
    lerobot_modality_meta: "LeRobotModalityMetadata",
    state_keys_full: list[str],
    state_mode_apply_keys: list[str] | None = None,
    base_stats: dict | None = None,
) -> dict:
    """Compute q01/q99 statistics for q[t]-q[t-1] state groups without changing absolute groups."""
    if base_stats is None:
        base_stats = calculate_dataset_statistics(parquet_paths)

    apply_keys = _normalize_state_mode_apply_keys(state_mode_apply_keys, state_keys_full)
    state_meta = lerobot_modality_meta.state
    grouped_slices: dict[str, list[tuple[int, int]]] = {}
    for state_key in apply_keys:
        if not state_key.startswith("state."):
            raise ValueError(f"Invalid state key {state_key}. Expected prefix 'state.'.")
        subkey = state_key.replace("state.", "", 1)
        if subkey not in state_meta:
            raise ValueError(f"State key missing in metadata: {state_key}")
        cfg = state_meta[subkey]
        grouped_slices.setdefault(cfg.original_key or subkey, []).append((cfg.start, cfg.end))

    state_stats = copy.deepcopy(base_stats)
    for state_col, slices in grouped_slices.items():
        values = []
        for parquet_path in tqdm(sorted(parquet_paths), desc="Collecting delta state stats"):
            data = pd.read_parquet(parquet_path)
            if state_col not in data.columns:
                raise ValueError(f"{state_col} not found in parquet columns.")
            matrix = np.stack(data[state_col])
            output = matrix.copy()
            episode_rows = (
                [np.asarray(rows, dtype=np.int64) for rows in data.groupby("episode_index", sort=False).indices.values()]
                if "episode_index" in data.columns
                else [np.arange(len(data), dtype=np.int64)]
            )
            for rows in episode_rows:
                for start, end in slices:
                    part = matrix[rows, start:end]
                    relative = np.zeros_like(part)
                    if len(part) > 1:
                        relative[1:] = part[1:] - part[:-1]
                    output[rows, start:end] = relative
            values.append(output)

        all_values = np.concatenate(values, axis=0).astype(np.float32)
...
def calculate_rel_action_statistics(
    ...
    for parquet_path in tqdm(sorted(list(parquet_paths)), desc="Collecting rel action stats"):
        ...
        trajectories = (
            (episode.reset_index(drop=True) for _, episode in file_data.groupby("episode_index", sort=False))
            if "episode_index" in file_data.columns
            else (file_data.reset_index(drop=True),)
        )
        for data in trajectories:
            ...
            for action_col, slice_list in action_col_slices.items():
                ...
                for base_index in range(trajectory_length):
                    ...
                    for a_slice, state_part_full, state_padding in prepared_slices:
                        action_part_chunk = action_chunk_full[:, a_slice[0] : a_slice[1]]
                        state_chunk = _get_chunk(
                            state_part_full,
                            np.array(state_indices) + base_index,
                            state_padding,
                        )
                        if action_part_chunk.shape[1] != state_chunk.shape[1]:
                            raise ValueError(f"Action/state dim mismatch for {action_col}:{a_slice}")

                        out = action_part_chunk - state_chunk[-1]
                        action_chunk_full[:, a_slice[0] : a_slice[1]] = out
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Infer LeRobot version from explicit config first, then metadata/file layout.
        self._lerobot_version = _infer_lerobot_version(Path(dataset_path), self.data_cfg)

        self._action_mode = None
        self._action_mode_state_map = {}
        self._action_mode_apply_keys = None
        self._state_mode = None
        self._state_mode_apply_keys = None
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        for new_key in le_modality_meta.video:
            ...
            width, height, channels = _video_resolution_from_feature(le_video_meta)
        ...
        state_mode = _normalize_state_mode(self.data_cfg.get("state_mode", "abs") if self.data_cfg else "abs")
        ...
        state_apply_keys = _normalize_state_mode_apply_keys(
            self.data_cfg.get("state_mode_apply_keys", None) if self.data_cfg else None,
            state_keys_full,
        )
        stats_cache_config = _build_stats_cache_config(
            ...
            state_mode=state_mode,
            state_mode_apply_keys=state_apply_keys,
        )
    ...
    def _init_state_mode(self) -> None:
        if self.data_cfg is None:
            self._state_mode = "abs"
            return

        self._state_mode = _normalize_state_mode(self.data_cfg.get("state_mode", "abs"))
        self._state_mode_apply_keys = _normalize_state_mode_apply_keys(
            self.data_cfg.get("state_mode_apply_keys", None)
        ) or None
    ...
    def _apply_action_mode(self, data: dict) -> dict:
        ...
        for action_key in action_keys:
            ...
            if self._action_mode == "delta":
                ...
                out[0] = action_values[0] - current_state
            elif self._action_mode == "rel":
                out = action_values - current_state
            else:
                out = action_values
    ...
    def _apply_state_mode(self, data: dict) -> dict:
        if self._state_mode in (None, "abs"):
            return data

        state_keys = self.modality_keys.get("state", [])
        relative_keys = set(self._state_mode_apply_keys or state_keys)
        for state_key in state_keys:
            if state_key not in data:
                continue
            state_values = np.asarray(data[state_key])
            if state_values.ndim != 2:
                raise ValueError(f"Expected 2D state array for {state_key}, got {state_values.shape}")
            if len(state_values) < 2:
                raise ValueError(f"State mode {self._state_mode} requires two frames for {state_key}")
            if state_key in relative_keys:
                data[state_key] = relative_arm_state_from_history(
                    state_values,
                    arm_indices=np.arange(state_values.shape[1]),
                )
            else:
                data[state_key] = state_values[-1:].copy()
        return data
    ...
    def _pack_sample(self, data: dict) -> dict:
        ...
        image_layout = self.data_cfg.get("image_layout", "native") if self.data_cfg is not None else "native"
        num_image_steps = len(data[self.modality_keys["video"][0]])
        if num_image_steps < 1:
            raise ValueError("video modality must contain at least one frame")
        step_images = []
        for time_index in range(num_image_steps):
            time_views = []
            for view_index, video_key in enumerate(self.modality_keys["video"]):
                if len(data[video_key]) != num_image_steps:
                    raise ValueError("all video views must contain the same number of time steps")
                image = data[video_key][time_index]
                if image_layout == "joint_songling":
                    image = preprocess_joint_songling_frame(image, view_index)
                elif image_layout == "joint_songling_top_padded":
                    image = preprocess_joint_songling_top_padded_frame(image, view_index)
                image = Image.fromarray(image).resize((224, 224))
                time_views.append(image)
            step_images.append(time_views)
        ...
        sample = {
            "action": action,
            "image": step_images[0] if len(step_images) == 1 else step_images,
            "lang": language,
            "robot_tag": self.tag
        }
    ...
    def get_step_data(self, trajectory_id: int, base_index: int) -> dict:
        ...
        data = self._apply_state_mode(data)
