...
def _open_pyav_single_thread(video_path: str):
    """Open a PyAV video with one FFmpeg decode thread per DataLoader worker."""
...
def get_frames_by_indices(
    ...
    if video_backend == "decord":
        ...
        container = None
        try:
            container, stream = _open_pyav_single_thread(video_path)
...
def get_all_frames(
    ...
    if video_backend == "decord":
        ...
        container, _ = _open_pyav_single_thread(video_path)
        frames = []
        for frame in container.decode(video=0):
