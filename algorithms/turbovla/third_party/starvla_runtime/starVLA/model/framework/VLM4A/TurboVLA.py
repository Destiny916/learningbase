...
class TurboVLAFramework(baseframework):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs) -> None:
        ...
        self.temporal_window_size = int(fw.vision.get("temporal_window_size", 1))
    ...
    def _core_config(fw) -> TurboVLAConfig:
        return TurboVLAConfig(
            ...
            vision=VisionEncoderConfig(
                ...
                temporal_window_size=int(fw.vision.get("temporal_window_size", 1)),
                position_embedding="learned_patch",
                encode_views_separately=False,
                frozen=bool(fw.vision.freeze_vision_encoder),
                gradient_checkpointing=bool(fw.vision.get("gradient_checkpointing", False)),
    ...
    def _as_view_list(self, images) -> list[Image.Image]:
        ...
        if len(views) < self.num_views:
            views.extend([views[-1]] * (self.num_views - len(views)))
        views = views[: self.num_views]
        return views

    def _as_temporal_view_list(self, images) -> list[list[Image.Image]]:
        if self.temporal_window_size == 1:
            return [self._as_view_list(images)]
        if not isinstance(images, (list, tuple)) or len(images) != self.temporal_window_size:
            raise ValueError(f"expected {self.temporal_window_size} image time steps")
        return [self._as_view_list(time_views) for time_views in images]

    def _model_inputs(self, examples: List[dict]):
        if not isinstance(examples, list):
            examples = [examples]
        device = next(self.parameters()).device
        views = [self._as_temporal_view_list(example["image"]) for example in examples]
        flat_images = [image for example_views in views for time_views in example_views for image in time_views]
        pixel_values = self.image_processor(images=flat_images, return_tensors="pt")["pixel_values"]
        pixel_values = pixel_values.view(
            len(examples), self.temporal_window_size, self.num_views, *pixel_values.shape[1:]
        ).to(device)
