...
def filter_compatible_checkpoint(checkpoint, target):
    """Keep checkpoint entries whose names and tensor shapes match ``target``.

    The released checkpoint has a 14D state projection, while the bread_xyz
    recipe expands the state input to 17D. ``strict=False`` still rejects
    shape mismatches, so incompatible entries must be omitted first.
    """
    target_state = target.state_dict() if hasattr(target, "state_dict") else target
    compatible = {}
    skipped = []
    for name, value in checkpoint.items():
        source_name = name[7:] if name.startswith("module.") else name
        replacements = (
            ("action_model.action_policy.action_head.", "action_head.decoder.action_projection."),
            ("action_model.action_policy.decoder.", "action_head.decoder.decoder."),
            ("action_model.action_policy.action_queries.", "action_head.decoder.action_queries."),
            ...
            ("dinov3.", "vision_encoder.backbone."),
        ...
        for source_prefix, target_prefix in replacements:
            if source_name.startswith(source_prefix):
                candidates.insert(0, target_prefix + source_name[len(source_prefix) :])
                break
        candidates.extend(
            candidate
            for candidate in list(candidates)
            if not candidate.startswith("model.")
            for candidate in (f"model.{candidate}",)
        )
        target_name = next((candidate for candidate in candidates if candidate in target_state), None)
        target_value = target_state.get(target_name) if target_name is not None else None
        if target_value is None or getattr(value, "shape", None) != getattr(target_value, "shape", None):
            skipped.append(name)
            continue
...
class TrainerUtils:
    ...
    def freeze_backbones(model, freeze_modules=""):
        ...
        directly freeze the specified submodules based on the relative module path list (patterns), no longer recursively find all submodule names:
          - patterns: read from config.trainer.freeze_modules, separated by commas to get the "relative path" list
            for example "text_encoder.bert, action_model",
            it means to freeze model.text_encoder.bert and model.action_model.
    ...
    def load_pretrained_backbones(model, checkpoint_path=None, reload_modules=None):
        ...
        if reload_modules:  # partial load
            ...
            try:
                compatible_checkpoint, skipped = filter_compatible_checkpoint(checkpoint, model)
                model.load_state_dict(compatible_checkpoint, strict=False)
                if skipped and is_main_process():
                    print(f"⚠️ skipped incompatible checkpoint keys ({len(skipped)}): {skipped}")
                if is_main_process():
