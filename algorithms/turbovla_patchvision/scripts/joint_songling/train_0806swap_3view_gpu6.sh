...
if [[ "${IN_TURBOVLA_CONTAINER:-0}" != "1" ]]; then
  ...
  exec docker run --rm --gpus "device=${selected_gpu}" --ipc=host \
    ...
    -e DINOV3_MODEL_PATH \
    -e RUN_ROOT_DIR \
    -e TURBOVLA_CONFIG_YAML="${TURBOVLA_CONFIG_YAML:-experiments/joint_songling/configs/0806swap_3view.yaml}" \
    -e WANDB_MODE="${WANDB_MODE:-disabled}" \
    -e PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}" \
    -e RUN_ID="${RUN_ID:-turbovla_0806swap_gripper_fixed_3view_gpu6}" \
    -e JOINT_SONGLING_OVERLAY_ROOT="${JOINT_SONGLING_OVERLAY_ROOT:-}" \
    -v "${repo_root}:${repo_root}" \
    -v /data:/data \
    -w "$repo_root" \
    "$image" bash scripts/joint_songling/train_0806swap_3view_gpu6.sh "$@"
fi
...
exec python3 -m accelerate.commands.launch --num_processes 1 \
  ...
  --config_yaml "${TURBOVLA_CONFIG_YAML:-experiments/joint_songling/configs/0806swap_3view.yaml}" \
  --run_root_dir "$RUN_ROOT_DIR" \
  --run_id "${RUN_ID:-turbovla_0806swap_gripper_fixed_3view_gpu6}" \
  "$@"
