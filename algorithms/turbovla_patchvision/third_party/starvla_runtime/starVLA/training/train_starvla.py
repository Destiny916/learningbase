...
def build_lr_scheduler(optimizer, cfg):
    scheduler_name = str(cfg.trainer.lr_scheduler_type)
    if scheduler_name == "warmup_constant_with_factor":
        warmup_steps = int(cfg.trainer.num_warmup_steps)
        start_factor = float(cfg.trainer.scheduler_specific_kwargs.get("warmup_start_factor", 0.1))

        def lr_lambda(current_step: int) -> float:
            if warmup_steps <= 0:
                return 1.0
            if current_step < warmup_steps:
                return start_factor + (1.0 - start_factor) * ((current_step + 1) / warmup_steps)
            return 1.0
...
def build_accelerator(cfg) -> Accelerator:
    use_deepspeed = bool(getattr(cfg.trainer, "use_deepspeed", True))
...
def setup_optimizer_and_scheduler(model, cfg) -> Tuple[torch.optim.Optimizer, torch.optim.lr_scheduler._LRScheduler]:
    ...
    optimizer = torch.optim.AdamW(
        ...
        foreach=cfg.trainer.optimizer.get("foreach", None),
        fused=cfg.trainer.optimizer.get("fused", None),
    )
    ...
    lr_scheduler = build_lr_scheduler(optimizer=optimizer, cfg=cfg)
...
class VLATrainer(TrainerUtils):
    ...
    def _init_wandb(self):
        ...
        if self.accelerator.is_main_process:
            ...
            if entity and entity != "your_wandb_entity":
            ...
            try:
                ...
                logger.warning(f"W&B init failed, continuing without W&B logging: {exc}")
