...
class TurboVLA(nn.Module):
    ...
    def __init__(self, config: TurboVLAConfig) -> None:
        ...
        self.temporal_window_size = int(config.vision.temporal_window_size)
        ...
        if self.temporal_window_size > 1:
            self.time_embedding = nn.Parameter(torch.zeros(1, self.temporal_window_size, 1, 1, hidden_dim))
    ...
    def _normalize_samples(self, samples: torch.Tensor | Mapping[str, torch.Tensor]) -> torch.Tensor:
        ...
        if pixel_values.shape[1] != self.temporal_window_size:
            raise ValueError(f"expected {self.temporal_window_size} image time steps, got {pixel_values.shape[1]}")
        if pixel_values.shape[2] != self.num_views:
            raise ValueError(f"expected {self.num_views} image views, got {pixel_values.shape[2]}")
        return pixel_values

    def _position_visual_tokens(self, tokens: torch.Tensor) -> torch.Tensor:
        if tokens.ndim != 5:
            raise ValueError(f"visual tokens must be [B,T,V,P,H], got {tuple(tokens.shape)}")
        if tokens.shape[1] != self.temporal_window_size or tokens.shape[2] != self.num_views:
            raise ValueError(f"unexpected temporal/view shape {tuple(tokens.shape[1:3])}")
        if self.config.vision.position_embedding == "learned_patch":
            if tokens.shape[3] != self.patch_position_embedding.shape[2]:
                raise ValueError(
                    f"configured patch position length {self.patch_position_embedding.shape[2]} "
                    f"does not match encoded length {tokens.shape[3]}"
                )
            ...
            positioned = tokens + scale * position + view
        else:
            view = self.view_embedding[:, None, :, None, :].to(device=tokens.device, dtype=tokens.dtype)
            positioned = tokens + view
        if self.time_embedding is not None:
            positioned = positioned + self.time_embedding.to(device=tokens.device, dtype=tokens.dtype)
        return positioned
    ...
    def encode_vision(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        return self._position_visual_tokens(tokens).flatten(1, 3)
