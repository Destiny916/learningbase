...
class DINOv3VisionEncoder(nn.Module):
    def __init__(self, config: VisionEncoderConfig) -> None:
        ...
        if config.gradient_checkpointing:
            if config.frozen:
                raise ValueError("gradient checkpointing requires a trainable vision encoder")
            if not getattr(self.backbone, "supports_gradient_checkpointing", True):
                raise ValueError("the configured vision backbone does not support gradient checkpointing")
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        has_time_axis = pixel_values.ndim == 6
        if pixel_values.ndim not in {5, 6}:
            raise ValueError(
                f"pixel_values must be [B,V,3,H,W] or [B,T,V,3,H,W], got {tuple(pixel_values.shape)}"
            )
        if not has_time_axis:
            pixel_values = pixel_values.unsqueeze(1)
        batch_size, time_steps, num_views = pixel_values.shape[:3]
        if time_steps != self.config.temporal_window_size:
            raise ValueError(f"expected {self.config.temporal_window_size} time steps, got {time_steps}")
        ...
        if self.config.encode_views_separately:
            tokens = torch.stack(
                [
                    self._encode_images(pixel_values[:, time_idx, view_idx])
            ...
            flat = pixel_values.flatten(0, 2)
            tokens = self._encode_images(flat)
        tokens = tokens.view(batch_size, time_steps, num_views, tokens.shape[-2], tokens.shape[-1])
        if has_time_axis:
            return tokens
        return tokens[:, 0]
