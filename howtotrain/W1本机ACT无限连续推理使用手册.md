...
```text
W1 左右头部相机、20D 机器人反馈、双手反馈、双腕临时黑图
  -> PC1:8890 观测与安全执行器
  -> 本机:8889 ACT 模型
  -> 返回 100x19 动作 chunk
  -> PC1 以 10 Hz 发布到 W1
...
- `observation.images.cam_hand_left`：左腕图像 key，当前使用全黑占位图。
- `observation.images.cam_hand_right`：右腕图像 key，当前使用全黑占位图。

三路图像的合同尺寸均为 `3x360x640`。PC1 向模型服务发送 `640x360x3` BGR
字节，模型服务先转为 RGB，再交给 checkpoint 自带的 processor。

双腕黑图只是当前尚未接入腕部相机时的临时接口占位，不是最终方案。后续接入
真实腕部相机后，需同时更新观测话题/图像链路，并使用真实腕图数据训练或微调
checkpoint；不能只把黑图换成真图就默认与原训练分布一致。
...
PC1: /home/dexforce/xwiz_black_wrist.log
...
若 `status` 已显示 `black_wrist_images`、`client_service` 正在运行且 `8890` 正在监听，不要重复启动。
...
- 左右头图、机器人状态、左右手反馈、左右腕临时占位图共 7 类观测齐全。
- 在尚未接入真实腕部相机的当前版本中，两路腕图必须为全黑占位图。
- 三个真机控制话题没有其他持续控制流。
