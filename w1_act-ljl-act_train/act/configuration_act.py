...
class ACTConfig(PreTrainedConfig):
    ...
    def __post_init__(self):
        super().__post_init__()
        ALLOWED_VISION_INPUT_NORM = {"auto", "imagenet", "siglip", "none"}
        if self.vision_input_norm not in ALLOWED_VISION_INPUT_NORM:
            raise ValueError(
                f"vision_input_norm must be one of {sorted(ALLOWED_VISION_INPUT_NORM)}, "
                f"got {self.vision_input_norm!r}"
            )
