...
class SimpleMoveSandboxPolicy(PolicyClientInterface):
    ...
    def try_take_response(self, request_id: int,
        ...
        if current_timepoint.time - self._response.request_meta.reqeust_time < 1.29:
            return None
        if request_id == 1 and self._response is not None:
            response = self._response
            self._response = None
            reqeust_time = response.request_meta.observation_sync_time
            duration = response.state_trajectory.end() - response.state_trajectory.begin()
            print(f'Response at {current_timepoint.time}, trajectory start time {reqeust_time}, duration {duration}')
            return response
        return None
...
if __name__ == "__main__":
    ...
    _sandbox_rollout_client(use_physical_time=True)
