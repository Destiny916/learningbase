...
def _expand_path(value: str) -> str:
    defaults = {
        "W1_ACT_CONTAINER_HOME": "/root/w1_act",
...
class W1RolloutConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        # Core
        self.policy_path = _expand_path(policy_path)
        self.device = device
...
class W1RolloutRobotDoF(object):
    ...
    def make_flatten_state(self,
                           q_body: np.ndarray,
                           left_scalar: Optional[np.ndarray],
                           right_scalar: Optional[np.ndarray],
                           left_q6: Optional[np.ndarray],
                           right_q6: Optional[np.ndarray]) -> Optional[np.ndarray]:
        ...
        for n in self.full_order:
            if n in self._config.body_canonical:
                state_vals.append(body_map.get(n, 0.0))
                continue

            if self._config.hand_input_mode == 'scalar':
                if n == self._config.left_scalar_name and 'left' in self._config.hand_sides:
                    state_vals.append(0.0 if left_scalar is None else float(left_scalar))
                    continue
                if n == self._config.right_scalar_name and 'right' in self._config.hand_sides:
                    state_vals.append(0.0 if right_scalar is None else float(right_scalar))
                    continue
            elif self._config.hand_input_mode == 'qpos6':
                if n in self._config.left_q6_names and 'left' in self._config.hand_sides:
                    i = self._config.left_q6_names.index(n)
                    v = 0.0 if left_q6 is None else float(left_q6[i])
                    state_vals.append(v)
                    continue
                if n in self._config.right_q6_names and 'right' in self._config.hand_sides:
                    i = self._config.right_q6_names.index(n)
