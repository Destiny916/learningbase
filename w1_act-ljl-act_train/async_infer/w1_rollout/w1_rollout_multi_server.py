...
# ===== Policy import =====
# Client sends raw observations only. Server handles lerobot preprocess/postprocess.
...
class InvokePolicyW1(SynchronizedPolicyClientInterface):
    ...
    def __init__(
            self,
            make_seed_batch_fn,
            ensure_remote_policy_ready_fn,
            remote_select_action_chunk_fn,
            decode_action_chunk_fn,
            nominal_dt: float,
    ):
        super().__init__()
        self._make_seed_batch_fn = make_seed_batch_fn
        self._ensure_remote_policy_ready_fn = ensure_remote_policy_ready_fn
        self._remote_select_action_chunk_fn = remote_select_action_chunk_fn
        self._decode_action_chunk_fn = decode_action_chunk_fn
    ...
    def __call__(self, invoke_info: PolicyClientInvokeInfo,
        ...
        try:
            batch = self._make_seed_batch_fn(obs_np, end_state)
        ...
        if not self._ensure_remote_policy_ready_fn():
            return PolicyClientResponse(
                request_meta=invoke_info.meta,
                state_trajectory=None,
                error_str="remote policy is not ready",
            )
        ...
        try:
            decoded_chunk = self._decode_action_chunk_fn(chunk)
        except Exception as exc:
            return PolicyClientResponse(
                request_meta=invoke_info.meta,
                state_trajectory=None,
...
class W1ACTFlexibleNode(Node):
    def __init__(self, config_path: Optional[str] = None,
        ...
        self._policy_executor = executor_not_own
        self._retired_policy_executors: List[ThreadPoolExecutor] = []
        ...
        # Remote policy only: client sends raw observations and receives decoded actions.
        self.network_client: Optional[NetworkClient] = None
        self._remote_policy_reset_required = True
        ...
        self._rollout_client = self._build_rollout_client()
    ...
    def _build_rollout_client(self) -> RolloutClientFunctor:
        obs_operation: RolloutClientObservationOperation = lambda t, is_invoke: self._client_take_obs(t, is_invoke)
        ...
        assert self._policy_executor is not None
        policy_generation = int(self._policy_generation)
        sync_policy_client = InvokePolicyW1(
            make_seed_batch_fn=self._make_seed_batch,
            ensure_remote_policy_ready_fn=lambda generation=policy_generation: self._ensure_remote_policy_ready(generation),
            remote_select_action_chunk_fn=lambda batch, generation=policy_generation: self._remote_select_action_chunk(
                batch,
                generation,
            ),
            decode_action_chunk_fn=self._decode_remote_action_chunk,
            nominal_dt=1.0 / max(1.0, float(self.config.policy_hz)),
        )
        policy_client = PolicyClientByFunctorOneActive(func=sync_policy_client, executor_now_owned=self._policy_executor)
        ...
        invoke_policy_operation = RolloutClientInvokePolicyOperation(
        ...
        rollout_cmd_config = RolloutClientCommandOption(
            state_dim_config=state_dim_config,
            merge_option=MergeTrajectoryOption(
                merge_type=MergeTrajectoryType.MergeByNearest,
                merge_blend_ratio=0.6,
            ),
        )
        return RolloutClientFunctor(
            command_option=rollout_cmd_config,
    ...
    def _publish_current_policy(self) -> None:
        if not hasattr(self, "pub_current_policy"):
            return
        msg = String()
    ...
    def _rotate_policy_executor(self) -> None:
        old_executor = self._policy_executor
        self._policy_generation += 1
        self._policy_executor = ThreadPoolExecutor(max_workers=1)
        if old_executor is not None:
            self._retired_policy_executors.append(old_executor)
    ...
    def _reset_runtime_for_policy_switch(self) -> None:
        self._client_cmd = None
        with self.plan_lock:
            self.plan_queue.clear()
            self.replan_in_progress = False
        self._rotate_policy_executor()
        self._rollout_client = self._build_rollout_client()
    ...
    def _switch_policy(self, policy_id: str, reason: str) -> tuple[bool, str]:
        ...
        self._reset_runtime_for_policy_switch()
        self._publish_current_policy()
    ...
    def _make_seed_batch(
            ...
            seed_vec_np: Optional[np.ndarray]
    ) -> Dict[str, np.ndarray]:
        obs = dict(obs_base)
        if seed_vec_np is not None:
    ...
    def _ensure_remote_connected(self, policy_generation: int) -> bool:
        ...
        if policy_generation != self._policy_generation:
            return False
        return self._connect_policy_server(self.active_policy_id)

    def _ensure_remote_policy_ready(self, policy_generation: int) -> bool:
        if policy_generation != self._policy_generation:
            return False
        if not self._ensure_remote_connected(policy_generation):
            return False
        if not self._remote_policy_reset_required:
            return True
        ...
        if ok:
            self._remote_policy_reset_required = False
        return ok
    ...
    def _decode_remote_action_chunk(self, chunk: np.ndarray) -> Optional[np.ndarray]:
        ...
        for i in range(chunk.shape[0]):
            act_np = self.robot_dof.action_to_np(np.asarray(chunk[i], dtype=np.float32))
            if act_np is None:
                return None
    ...
    def _spawn_replan(self, batch: Dict[str, np.ndarray], sync: bool = False) -> None:
        ...
        def worker():
            actions_local: List[np.ndarray] = []
            if not self._ensure_remote_policy_ready():
                self.get_logger().error("remote policy is not ready in worker.")
                self.replan_in_progress = False
                return
            ...
            try:
                ...
                decoded_chunk = self._decode_remote_action_chunk(chunk)
                if decoded_chunk is None:
                    self.replan_in_progress = False
                    return
                actions_local.extend(decoded_chunk)
            except Exception as e:
    ...
    try:
        ...
        if hasattr(node, 'network_client'):
            try:
                ...
                pass
        if hasattr(node, '_policy_executor') and node._policy_executor is not None:
            node._policy_executor.shutdown(wait=False, cancel_futures=True)
        if hasattr(node, '_retired_policy_executors'):
            for retired_executor in node._retired_policy_executors:
