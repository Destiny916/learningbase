...
try:
    ...
    raise ImportError("Import ACTPolicy failed; ensure act/modeling_act.py in PYTHONPATH") from e
...
class W1ACTFlexibleNode(Node):
    ...
    def _make_seed_batch(
        self,
        batch_base: Dict[str, torch.Tensor],
        ...
        for k, v in batch_base.items():
            if k == self.state_key:
                out[k] = torch.from_numpy(
                    seed_vec_np[None, ...].astype(np.float32)
                ).to(self.device)
            else:
                out[k] = v
        return out
        ...
        def worker():
            ...
            with self.plan_lock:
                ...
                if actions_local and do_blend:
                    offset = prev_tail - actions_local[0]
    ...
    def timer_infer(self) -> None:
        ...
        batch: Dict[str, torch.Tensor] = {k: torch.from_numpy(v).to(self.device) for k, v in batch_np.items()}
