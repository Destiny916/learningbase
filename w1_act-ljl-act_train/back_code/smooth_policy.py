...
class W1ACTDirectNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # ---- NEW: Arm smoothing time-constant (ms) ----
        ...
        # —— Chunk 边界平滑相关参数 ——
        self.declare_parameter('chunk_period_steps', 30)  # 训练时的action chunk=30
        self.declare_parameter('fade_frames', 12)  # 跨多少发布帧做淡入，建议6~12
        self.declare_parameter('jump_thresh', 0.12)  # 自动检测跳变的阈值（单位=关节值）

        # 读取
        self.chunk_period = int(
            self.get_parameter(
                'chunk_period_steps').get_parameter_value().integer_value)
        self.fade_frames = int(
            self.get_parameter(
                'fade_frames').get_parameter_value().integer_value)
        self.jump_thresh = float(
            self.get_parameter(
                'jump_thresh').get_parameter_value().double_value)

        # 运行时状态
        self._arm_step_in_period = 0
        self._arm_fade_left = 0
        self._arm_fade_from = None
        self._arm_fade_to = None
        # ---------- Read parameters ---------- #
    ...
    def timer_infer(self) -> None:
        ...
        act_pub = act[self.publish_keep_index].astype(np.float32)

        # === ARM 平滑：EMA + 边界 cross-fade ===
        now_ts = now_sec()
        vec = act_pub
        ...
        if self._arm_prev is None:
            ...
            alpha = dt_arm / (self.arm_tau + dt_arm)  # EMA 主平滑
            ema_target = (1.0 - alpha) * self._arm_prev + alpha * vec

            # —— 边界触发：优先用“固定周期”，否则用“自动检测大跳变” ——
            boundary_hit = False
            if self.chunk_period > 0:
                # 固定周期：每过 chunk_period 步触发
                self._arm_step_in_period = (self._arm_step_in_period +
                                            1) % self.chunk_period
                boundary_hit = (self._arm_step_in_period == 0)
            else:
                # 自动检测：均值变化超过阈值视为“边界”
                mean_jump = float(np.mean(np.abs(vec - self._arm_prev)))
                boundary_hit = (mean_jump > self.jump_thresh)

            # 如触发边界，则开始跨帧淡入（覆盖EMA的瞬时突变）
            if boundary_hit:
                self._arm_fade_left = max(1, self.fade_frames)
                self._arm_fade_from = self._arm_prev.copy()
                self._arm_fade_to = ema_target.copy()

            # 执行 cross-fade；否则直接采纳 EMA
            if getattr(self, '_arm_fade_left', 0) > 0:
                k = (self.fade_frames - self._arm_fade_left + 1) / float(
                    self.fade_frames)
                self._arm_prev = (
                    1.0 - k) * self._arm_fade_from + k * self._arm_fade_to
                self._arm_fade_left -= 1
            else:
                self._arm_prev = ema_target
