...
class W1ACTFlexibleNode(Node):
    def __init__(self, config_path: Optional[str] = None) -> None:
        ...
        self.cam_high_left_topic = str(self.raw_config.get("cam_high_left_topic", "/camera/head_left/image_raw"))
        self.cam_high_right_topic = str(self.raw_config.get("cam_high_right_topic", "/camera/head_right/image_raw"))
        ...
        if "observation.images.cam_high_left" in self.config.image_keys:
        ...
        if "observation.images.cam_high_right" in self.config.image_keys:
        ...
        self.create_subscription(EEJointFeedback, self.config.left_hand_qpos6_topic, self.cb_qpos6_left, q_reliable)
        self.create_subscription(EEJointFeedback, self.config.right_hand_qpos6_topic, self.cb_qpos6_right, q_reliable)
    ...
    def _update_head_frame(self, side: str, bgr: np.ndarray) -> None:
        t = now_sec()
        need_left = "observation.images.cam_high_left" in self.config.image_keys
        need_right = "observation.images.cam_high_right" in self.config.image_keys

        if side == 'left':
            self.head_left_buf.append((t, bgr))
            left_bgr = bgr
            right_bgr = nearest(self.head_right_buf, t, float(self.config.tolerance_ms) / 1000.0) if need_right else None
            if need_right and right_bgr is None:
                return
        else:
            self.head_right_buf.append((t, bgr))
            right_bgr = bgr
            left_bgr = nearest(self.head_left_buf, t, float(self.config.tolerance_ms) / 1000.0) if need_left else None
            if need_left and left_bgr is None:
                return
