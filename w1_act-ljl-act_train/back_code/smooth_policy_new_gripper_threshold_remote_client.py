...
def bgr_to_chw_rgb_resized(img_bgr: np.ndarray, size: Tuple[int, int]) -> np.ndarray:
...
class W1ACTFlexibleNode(Node):
    def __init__(self) -> None:
        ...
        # Buffers
        self.body_buf: deque = deque(maxlen=400)
        ...
        self.scalar_left_buf: deque  = deque(maxlen=200)
        self.scalar_right_buf: deque = deque(maxlen=200)
        self.qpos6_left_buf: deque   = deque(maxlen=200)
        self.qpos6_right_buf: deque  = deque(maxlen=200)
        ...
        self.blend_steps = 30  # 过渡长度(帧数)，越小越"看不出来"
    ...
    def cb_joint(self, msg: String) -> None:
        ...
        # 2. 时间戳：你原来已经不用 header 了，就是 now_sec()
        t = now_sec()
        q_in = np.asarray(joint_pos, dtype=np.float32)

        # 3. 构建 body_index_map：
        #    之前是用 msg.name，现在我们知道 joint_position 的顺序固定是 JOINT_NAMES
    ...
    def _make_seed_batch(
        self,
        batch_base: Dict[str, np.ndarray],
        seed_vec_np: np.ndarray
    ) -> Dict[str, np.ndarray]:
        out: Dict[str, np.ndarray] = {}
        for k, v in batch_base.items():
            if k == self.state_key:
                out[k] = seed_vec_np[None, ...].astype(np.float32, copy=True)
            else:
    ...
    def _remote_select_action(self, batch: Dict[str, np.ndarray]) -> Optional[np.ndarray]:
        if not self._ensure_remote_connected():
            self.get_logger().error("Reconnect remote policy server failed.")
            return None
        payload = {
            "batch": {k: np.asarray(v, dtype=np.float32) for k, v in batch.items()}
        }
        ...
        if response.get("status") != "success":
            ...
            return None
        return self._action_to_np(response.get("action"))
    
    def _remote_select_action_chunk(self, batch: Dict[str, np.ndarray]) -> Optional[np.ndarray]:
        """一次请求拿整个 action chunk，返回 (n_steps, action_dim) 的 np.ndarray."""
        if not self._ensure_remote_connected():
            self.get_logger().error("Reconnect remote policy server failed.")
            return None
        payload = {"batch": {k: np.asarray(v, dtype=np.float32) for k, v in batch.items()}}
        ...
        # 校验每个 step 的 dim
        if actions.ndim != 2 or actions.shape[1] != self.full_dim:
            self.get_logger().error(
                f"Chunk dim mismatch: got {actions.shape}, expected (*, {self.full_dim})"
            )
            return None
        return actions
    def _spawn_replan(self, batch: Dict[str, np.ndarray], sync: bool = False) -> None:
        ...
        def worker():
            ...
            #     t0 = now_sec()
            #     act_np = self._remote_select_action(batch)
            #     t1 = now_sec()
            #     infer_dt = t1 - t0
            #     if infer_dt > 0.1:
            #         self.get_logger().info(
            #             f'[async] heavy remote inference {infer_dt:.3f}s (worker)'
            #         )
            #     if act_np is None:
            #         break
            ...
            t1 = now_sec()
            # if t1 - t0 > 0.1:
            self.get_logger().info(f'[async] remote chunk infer {t1-t0:.3f}s')

            if chunk is None:
                self.replan_in_progress = False
                return

            actions_local = [chunk[i] for i in range(chunk.shape[0])]
            ...
            with self.plan_lock:
                ...
                if actions_local and do_blend:
                    ...
                    for i in range(Nblend):
                        alpha = 1.0 - (i / denom)
                        # alpha = 0.5 * (1.0 + np.cos(np.pi * i / denom))
                        actions_local[i] = (
    ...
    def timer_infer(self) -> None:
        # ═══════════════════════════════════════════════════════
        # 快速路径：先从队列拿一帧发出去，大多数帧走这里就结束了
        # ═══════════════════════════════════════════════════════
        with self.plan_lock:
            qlen       = len(self.plan_queue)
            busy       = self.replan_in_progress
            act_np     = self.plan_queue.popleft() if self.plan_queue else None
        ...
        # 队列还充裕，不需要 replan → 直接返回，本帧结束
        need_replan = (qlen_after <= self.replan_trigger and not busy_after) \
                or (qlen == 0 and not busy)
        if not need_replan:
            return

        # ═══════════════════════════════════════════════════════
        # 慢速路径：只有需要 replan 时才做图像预处理 + 状态打包
        # ═══════════════════════════════════════════════════════
        with self.frame_lock:
        ...
        right_scalar = nearest(self.scalar_right_buf, t_anchor, self.tolerance_s)
        ...
        for n in self.full_order:
            ...
            if self.hand_input_mode == 'scalar':
                ...
                if n in self.left_q6_names and 'left' in self.hand_sides:
                    ...
                    state_vals.append(0.0 if left_q6 is None else float(left_q6[i]))
                    continue
                if n in self.right_q6_names and 'right' in self.hand_sides:
                    i = self.right_q6_names.index(n)
                    state_vals.append(0.0 if right_q6 is None else float(right_q6[i]))
                    continue
        ...
        for key in self.image_keys:
            if key == "observation.images.cam_high_left":
                if tf.left_bgr is None: return
                batch_np[key] = bgr_to_chw_rgb_resized(tf.left_bgr, (head_W, head_H))[None, ...]
            elif key == "observation.images.cam_high_right":
                if tf.right_bgr is None: return
                batch_np[key] = bgr_to_chw_rgb_resized(tf.right_bgr, (head_W, head_H))[None, ...]
            elif key == "observation.images.cam_hand_left":
                bgr = nearest(self.hand_left_buf, t_anchor, self.tolerance_s)
                if bgr is None: return
                batch_np[key] = bgr_to_chw_rgb_resized(bgr, (hand_W, hand_H))[None, ...]
        ...
        batch: Dict[str, np.ndarray] = {
            k: np.asarray(v, dtype=np.float32) for k, v in batch_np.items()
        }

        # ---------- replan 决策 ----------
        if qlen == 0 and act_np is None:
            # 队列完全空了，同步阻塞补一次（兜底，正常不该走到这里）
            self.get_logger().warn("plan_queue empty, blocking replan!")
            ...
            # 正常情况：队列快用完了，异步补货，seed 用队列末尾的未来姿态
            with self.plan_lock:
                future_seed = (
                    self.plan_queue[-1].astype(np.float32, copy=True)
                    if self.plan_queue
                    else state_vec.astype(np.float32, copy=True)
                )
