...
try:
    ...
    raise ImportError("Import PI05Policy/ProcessorPipeline failed; check your lerobot install") from e
...
class W1PI05FlexibleNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter('task', 'purple_candy')
        ...
        self.get_logger().info(f'Loading PI05Policy from {self.policy_path} on {self.device} ...')
        self.policy = PI05Policy.from_pretrained(self.policy_path, local_files_only=True)
        ...
        self._log_inference_preprocess_chain()

        cfg.n_action_steps = min(cfg.n_action_steps, cfg.chunk_size)
        self.policy.reset()
        self.get_logger().info('Policy loaded.')
        self.get_logger().info(
            f"[PI05] chunk_size={cfg.chunk_size}, n_action_steps={cfg.n_action_steps}"
        )
        ...
        def worker():
            ...
            with torch.no_grad():
                ...
                try:
                    ...
                    self.get_logger().error(f'policy.predict_action_chunk failed in worker: {e}')
                ...
                if infer_dt > 0.1:
                    self.get_logger().info(
                        f'[async] heavy chunk inference {infer_dt:.3f}s (worker)'
                    )

                if chunk.ndim == 3:
                    chunk = chunk[0]
                if chunk.ndim != 2:
                    self.get_logger().error(f'predict_action_chunk returns bad shape: {tuple(chunk.shape)}')
                    self.replan_in_progress = False
                    return

                n_steps = min(self.horizon_N, int(chunk.shape[0]))
                for i in range(n_steps):
                    a = self._postprocess_action(chunk[i])
