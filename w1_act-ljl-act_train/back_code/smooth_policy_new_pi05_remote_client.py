...
from end_effector_interfaces.msg import EEJointControl, EEJointControlMode, EEJointFeedback
...
def bgr_to_hwc_rgb_preprocessed(
    img_bgr: np.ndarray, size: Tuple[int, int], center_crop: bool = True
) -> np.ndarray:
    """BGR HxWx3 -> optional center-crop -> RGB HWC uint8 resized to (W,H)."""
    import cv2
...
class W1PI05FlexibleNode(Node):
    def __init__(self) -> None:
        super().__init__('w1_pi05_flexible_node')
        ...
        self.declare_parameter('remote_replan_trigger', 15)
        ...
        self.declare_parameter('image_center_crop', True)
        self.declare_parameter('model_input_width', 256)
        ...
        self.declare_parameter('task', 'purple candy')
        self.declare_parameter('robot_type', 'w1')
        ...
        self.image_center_crop = bool(gp('image_center_crop').get_parameter_value().bool_value)
        self.model_input_size = (
            int(gp('model_input_width').get_parameter_value().integer_value),
            int(gp('model_input_height').get_parameter_value().integer_value),
        )
        self.state_key       = gp('state_key').get_parameter_value().string_value
        self.task: str = gp('task').get_parameter_value().string_value
        self.robot_type: str = gp('robot_type').get_parameter_value().string_value
        ...
        self.create_subscription(EEJointFeedback, self.left_qpos6_topic,  self.cb_qpos6_left,  q_reliable)
        self.create_subscription(EEJointFeedback, self.right_qpos6_topic, self.cb_qpos6_right, q_reliable)

        # Lightweight client mode: no local policy/preprocessor/tokenizer/postprocessor.
        self.device = torch.device(self.device_str if (self.device_str == 'cuda' and torch.cuda.is_available()) else 'cpu')
        self.preprocessor = None
        self.postprocessor = None
        self.get_logger().info("Client lightweight mode enabled: only packs raw observation + task.")
        ...
        self.get_logger().info(
            f"[Remote PI05] horizon_N={self.horizon_N}, "
    ...
    def cb_qpos6_left(self, msg: EEJointFeedback) -> None:
        arr = np.asarray(msg.position, dtype=np.float32)
        if arr.shape[0] >= 6:
            self.qpos6_left_buf.append((now_sec(), arr[:6].copy()))

    def cb_qpos6_right(self, msg: EEJointFeedback) -> None:
        arr = np.asarray(msg.position, dtype=np.float32)
        if arr.shape[0] >= 6:
    ...
    def _format_processor_steps(self, pipeline: Any) -> List[str]:
        lines: List[str] = []
        ...
        for i, step in enumerate(steps):
            ...
            if hasattr(step, "norm_map"):
                extras.append(f"norm_map={getattr(step, 'norm_map')}")
            if hasattr(step, "resize_size") and getattr(step, "resize_size") is not None:
                extras.append(f"resize_size={getattr(step, 'resize_size')}")
            if hasattr(step, "crop_params_dict") and getattr(step, "crop_params_dict"):
                extras.append("crop_params_dict=<set>")
            if hasattr(step, "device"):
                extras.append(f"device={getattr(step, 'device')}")
            extra_str = f" ({', '.join(extras)})" if extras else ""
            lines.append(f"{i:02d}. {name}{extra_str}")
        return lines

    def _log_inference_preprocess_chain(self) -> None:
        self.get_logger().info(
            f"[image preprocess] center_crop={self.image_center_crop}, "
            f"model_input_size={self.model_input_size}"
        )
        pre_lines = self._format_processor_steps(self.preprocessor)
        post_lines = self._format_processor_steps(self.postprocessor)
        self.get_logger().info("[preprocessor steps]\n" + "\n".join(pre_lines))
        self.get_logger().info("[postprocessor steps]\n" + "\n".join(post_lines))

    def _init_language_tokenizer_fallback(self) -> None:
        self._language_tokenizer = None
        self._language_tokenizer_max_length = 200
        try:
            preproc_cfg_path = os.path.join(self.policy_path, "policy_preprocessor.json")
            with open(preproc_cfg_path, "r", encoding="utf-8") as f:
                preproc_cfg = json.load(f)
            tokenizer_name = "google/paligemma-3b-pt-224"
            for step in preproc_cfg.get("steps", []):
                if step.get("registry_name") == "tokenizer_processor":
                    cfg = step.get("config", {})
                    tokenizer_name = cfg.get("tokenizer_name", tokenizer_name)
                    self._language_tokenizer_max_length = int(cfg.get("max_length", 200))
                    break
            from transformers import AutoTokenizer

            self._language_tokenizer = AutoTokenizer.from_pretrained(
                tokenizer_name, local_files_only=True
            )
            self.get_logger().info(
                f"Loaded tokenizer fallback: {tokenizer_name} "
                f"(max_length={self._language_tokenizer_max_length})"
            )
        except Exception as e:
            self.get_logger().warn(f"Tokenizer fallback unavailable: {e}")

    def _ensure_language_inputs(self, batch_np: Dict[str, np.ndarray], batch_t: Dict[str, Any]) -> None:
        token_key = "observation.language.tokens"
        mask_key = "observation.language.attention_mask"
        if token_key in batch_np and mask_key in batch_np:
            return
        if self._language_tokenizer is None:
            return

        task_text = str(self.task).strip()
        if "task" in batch_t:
            raw_task = batch_t["task"]
            if isinstance(raw_task, torch.Tensor):
                pass
            else:
                task_arr = np.asarray(raw_task)
                if task_arr.size > 0:
                    task_text = str(task_arr.reshape(-1)[0]).strip()
        if task_text == "":
            task_text = "do task"

        encoded = self._language_tokenizer(
            task_text,
            max_length=self._language_tokenizer_max_length,
            padding="max_length",
            truncation=True,
            return_tensors="np",
        )
        if token_key not in batch_np and "input_ids" in encoded:
            batch_np[token_key] = np.asarray(encoded["input_ids"], dtype=np.int64)
        if mask_key not in batch_np and "attention_mask" in encoded:
            batch_np[mask_key] = np.asarray(encoded["attention_mask"], dtype=np.bool_)

        if token_key in batch_np and mask_key in batch_np:
            self.get_logger().warn(
                "Preprocessor did not output PI05 language tensors; used tokenizer fallback from task."
            )


    def _preprocess_observation(self, obs_np: Dict[str, np.ndarray]) -> Dict[str, torch.Tensor]:
        """Prepare observation dict for PI05Policy.predict_action_chunk().
    ...
    def _make_seed_batch(
        ...
        for key, value in obs.items():
            ...
            if not is_numeric_or_bool:
                self.get_logger().warn(
                    f"Skip non-numeric observation key={key} dtype={arr.dtype}"
                )
                continue
            if arr.dtype == np.float64:
                arr = arr.astype(np.float32)
            batch_np[key] = arr
        batch_np["task"] = str(self.task)
        batch_np["robot_type"] = str(self.robot_type)
        return batch_np

    def _sanitize_remote_batch(self, batch: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        payload_batch: Dict[str, Any] = {}
        token_key = "observation.language.tokens"
        mask_key = "observation.language.attention_mask"
        for key, value in batch.items():
            if key == ACTION or key == "action" or value is None:
                continue
            if key in ("task", "robot_type"):
                payload_batch[key] = str(value)
                continue
            arr = np.asarray(value)
            is_numeric_or_bool = np.issubdtype(arr.dtype, np.number) or np.issubdtype(
                arr.dtype, np.bool_
            ...
            if key == token_key:
                ...
                if np.issubdtype(arr.dtype, np.bool_):
                    ...
                    arr = (arr > 0.5).astype(np.bool_, copy=False)
                else:
                    arr = (arr != 0).astype(np.bool_, copy=False)
            if arr.dtype == np.float64:
                arr = arr.astype(np.float32)
            payload_batch[key] = arr
    ...
    def _remote_select_action_chunk(
        ...
        if not self._ensure_remote_connected():
            ...
            return None
        payload_batch = self._sanitize_remote_batch(batch)
        # payload = {
        #     "batch": payload_batch,
        ...
        def worker():
            ...
            try:
                ...
                for i in range(chunk.shape[0]):
                    act_np = self._action_to_np(chunk[i])
                    if act_np is None:
                        break
    ...
    def timer_infer(self) -> None:
        ...
        head_W, head_H = int(self.model_input_size[0]), int(self.model_input_size[1])
        hand_W, hand_H = int(self.model_input_size[0]), int(self.model_input_size[1])

        for key in self.image_keys:
            if key == "observation.images.cam_high_left":
                if tf.left_bgr is None:
                    return
                rgb = bgr_to_hwc_rgb_preprocessed(
                    tf.left_bgr, (head_W, head_H), center_crop=self.image_center_crop
                )
                obs_np[key] = rgb
                ...
                if tf.right_bgr is None:
                    return
                rgb = bgr_to_hwc_rgb_preprocessed(
                    tf.right_bgr, (head_W, head_H), center_crop=self.image_center_crop
                )
                obs_np[key] = rgb
                ...
                if bgr is None:
                    return
                rgb = bgr_to_hwc_rgb_preprocessed(
                    bgr, (hand_W, hand_H), center_crop=self.image_center_crop
                )
                obs_np[key] = rgb
