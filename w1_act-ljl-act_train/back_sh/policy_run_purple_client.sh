...
W1_PURPLE_POLICY_PATH="${W1_PURPLE_POLICY_PATH:-${W1_ACT_CONTAINER_HOME}/checkpoints/new_year/purple_siglip_freeze_018/pretrained_model}"

# （可选）如果本次不使用手的标量输入，这一行可以删掉或保留：
taskset -c 7 python3 inference_codes/hand_scalar.py &

# 启动 10 维身体输入的 ACT 推理节点
taskset -c 7 python3 inference_codes/smooth_policy_new_act_remote_client.py --ros-args \
  -p remote_server_host:=192.168.20.88 \
  -p remote_server_port:=8899 \
  -p policy_path:="${W1_PURPLE_POLICY_PATH}" \
  ...
  -p set_left_hand_qpos6_topic:=/control/ee/left \
  -p set_right_hand_qpos6_topic:=/control/ee/right \
