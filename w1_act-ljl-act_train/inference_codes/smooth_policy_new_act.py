...
def bgr_to_hwc_rgb_resized(img_bgr: np.ndarray, size: Tuple[int, int]) -> np.ndarray:
    ...
    # x = cv2.resize(img_bgr, (tw, th), interpolation=cv2.INTER_NEAREST)
...
try:
    ...
    from lerobot.utils.constants import ACTION, OBS_IMAGES, OBS_STATE  # 可选，常量

except Exception as e:
    raise ImportError("Import ACTPolicy/ProcessorPipeline failed; check your lerobot install") from e
...
class W1ACTFlexibleNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter('head_image_source', 'usb')  # usb | ros
        ...
        # 手部相机对应的 topic
        self.declare_parameter('cam_high_left_topic', '/camera/left_eye_resize')
        ...
        self.declare_parameter('cam_hand_right_topic', '/camera/right/image_raw')
        ...
        self.head_image_source = gp('head_image_source').get_parameter_value().string_value.lower().strip()
        self.urdf_path: str = gp('urdf_path').get_parameter_value().string_value.strip()
        if self.head_image_source not in ('usb', 'ros'):
            raise ValueError("head_image_source must be 'usb' or 'ros'")
        ...
        self.cam_high_left_topic  = self.get_parameter('cam_high_left_topic').get_parameter_value().string_value
        self.cam_high_right_topic = self.get_parameter('cam_high_right_topic').get_parameter_value().string_value
        ...
        self.joint_limit_map: Dict[str, Tuple[float, float]] = self._load_revolute_joint_limits(self.urdf_path)
        (
            self.full_order_lower,
            self.full_order_upper,
            self.full_order_clip_mask,
        ) = self._build_joint_limit_arrays(self.full_order)
        ...
        self.get_logger().info(f"[head image] source={self.head_image_source}")
        if self.joint_limit_map:
            clipped_names = [n for n in self.full_order if n in self.joint_limit_map]
            missing_names = [n for n in self.full_order if n not in self.joint_limit_map]
            self.get_logger().info(
                f"[urdf] loaded {len(self.joint_limit_map)} revolute joint limits from {self.urdf_path}"
            )
            self.get_logger().info(
                f"[urdf] clip-enabled outputs ({len(clipped_names)}): {clipped_names}"
            )
            if missing_names:
                self.get_logger().warn(
                    f"[urdf] outputs without matching revolute limit ({len(missing_names)}): {missing_names}"
                )
        else:
            self.get_logger().info("[urdf] no revolute joint limits loaded; action clipping disabled")
        ...
        if self.head_image_source == 'ros':
            self.create_subscription(Image, self.cam_high_left_topic, self.cb_img_head_left, q_img)
            self.get_logger().info(f"Subscribe head-left image: {self.cam_high_left_topic}")
            self.create_subscription(Image, self.cam_high_right_topic, self.cb_img_head_right, q_img)
            self.get_logger().info(f"Subscribe head-right image: {self.cam_high_right_topic}")
        ...
        self.get_logger().info(f'Loading ACTPolicy from {self.policy_path} on {self.device} ...')
        self.policy = ACTPolicy.from_pretrained(self.policy_path, local_files_only=True)
        ...
        # <= 这个阈值时触发异步补货 (例如15)
        self.replan_trigger: int = max(15, self.horizon_N // 2)
    ...
    def _load_revolute_joint_limits(self, urdf_path: str) -> Dict[str, Tuple[float, float]]:
        if not urdf_path:
            return {}

        resolved = os.path.abspath(os.path.expanduser(urdf_path))
        ...
        try:
            ...
            raise RuntimeError(f"Failed to parse URDF {resolved}: {e}") from e

        limits: Dict[str, Tuple[float, float]] = {}
        skipped_no_limit: List[str] = []
        for joint in root.findall("joint"):
            ...
            try:
                ...
                raise RuntimeError(
                    f"Invalid URDF limit for joint {name} in {resolved}: "
                    f"lower={lower_raw}, upper={upper_raw}"
                ) from e

            if lower > upper:
                raise RuntimeError(
                    f"URDF joint {name} has lower > upper in {resolved}: {lower} > {upper}"
                )
            limits[name] = (lower, upper)

        if skipped_no_limit:
            self.get_logger().warn(
                f"[urdf] revolute joints without usable lower/upper limits: {skipped_no_limit}"
            )

        self.urdf_path = resolved
        return limits

    def _build_joint_limit_arrays(
        self,
        names: List[str],
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        lower = np.full(len(names), -np.inf, dtype=np.float32)
        upper = np.full(len(names), np.inf, dtype=np.float32)
        mask = np.zeros(len(names), dtype=bool)
        for i, name in enumerate(names):
            if name not in self.joint_limit_map:
                continue
            lo, hi = self.joint_limit_map[name]
            lower[i] = np.float32(lo)
            upper[i] = np.float32(hi)
            mask[i] = True
        return lower, upper, mask

    def _clip_action_vector(self, act_np: np.ndarray) -> np.ndarray:
        if getattr(self, "full_order_clip_mask", None) is None:
            return act_np
        if not np.any(self.full_order_clip_mask):
            return act_np
        clipped = act_np.astype(np.float32, copy=True)
        clipped[self.full_order_clip_mask] = np.clip(
            clipped[self.full_order_clip_mask],
            self.full_order_lower[self.full_order_clip_mask],
            self.full_order_upper[self.full_order_clip_mask],
        )
        return clipped

    # ---------- Camera ----------
    def _update_head_frame(self, side: str, bgr: np.ndarray) -> None:
        t = now_sec()
        if side == 'left':
            self.head_left_buf.append((t, bgr))
            other = nearest(self.head_right_buf, t, self.tolerance_s)
            if other is None:
                return
            left_bgr = bgr
            right_bgr = other
            ...
            other = nearest(self.head_left_buf, t, self.tolerance_s)
            if other is None:
                return
            left_bgr = other
            right_bgr = bgr
    ...
    def cb_img_head_left(self, msg: Image) -> None:
        try:
            ...
            self.get_logger().warn(f"cv_bridge head-left fail: {e}")
            return
        self._update_head_frame('left', bgr)

    def cb_img_head_right(self, msg: Image) -> None:
        try:
            bgr = self.bridge.imgmsg_to_cv2(msg, desired_encoding="bgr8")
        except Exception as e:
            self.get_logger().warn(f"cv_bridge head-right fail: {e}")
            return
        self._update_head_frame('right', bgr)
    ...
    def _action_to_np(self, action) -> Optional[np.ndarray]:
        ...
        return self._clip_action_vector(act.astype(np.float32, copy=True))
    ...
    def _publish_action_vector(self, act_np: np.ndarray) -> None:
        act_np = self._clip_action_vector(act_np)
        ...
        def make_hand_msg(values: np.ndarray) -> EEJointControl:
            msg = EEJointControl()
            msg.header.stamp = self.get_clock().now().to_msg()
            msg.mode = EEJointControlMode.POSITION
            msg.joint_names = list(self.hand_joint_name)
            msg.values = [float(x) for x in values]
            return msg
        ...
        if self.hand_input_mode == 'scalar':
            if 'left' in self.hand_sides and self.idx_left_scalar is not None:
                ...
                self.pub_set_left_qpos6.publish(make_hand_msg(q6))
            ...
            if 'right' in self.hand_sides and self.idx_right_scalar is not None:
                ...
                q6 = self.map_scalar_to_qpos6(s)
                self.pub_set_right_qpos6.publish(make_hand_msg(q6))

        elif self.hand_input_mode == 'qpos6':
            if 'left' in self.hand_sides and self.slice_left_q6 is not None:
                a, b = self.slice_left_q6
                v = act_np[a:b]
                self.pub_set_left_qpos6.publish(make_hand_msg(v))
            if 'right' in self.hand_sides and self.slice_right_q6 is not None:
                a, b = self.slice_right_q6
                v = act_np[a:b]
                self.pub_set_right_qpos6.publish(make_hand_msg(v))
    ...
    def _spawn_replan(self, batch: Dict[str, torch.Tensor], sync: bool = False) -> None:
            ...
            with torch.no_grad():
                for _ in range(self.horizon_N):
                    ...
                    try:
                        ...
                        self.get_logger().error(f'policy.select_action failed in worker: {e}')
                        break
                    t1 = now_sec()
                    infer_dt = t1 - t0
                    if infer_dt > 0.1:
    ...
    def timer_infer(self) -> None:
        ...
        if tf is None:
            return
        t_anchor = tf.t
        # print("head anchor", t_anchor)
        # 2) proprio 状态 (按照 full_order)
        state_vals: List[float] = []
        # print("body", self.body_buf[-1][0])
        if len(self.body_order) > 0:
...
def main() -> None:
    ...
    try:
        ...
        if hasattr(node, '_cap_stop'):
            node._cap_stop = True
        if hasattr(node, '_cap_thread') and node._cap_thread is not None and node._cap_thread.is_alive():
