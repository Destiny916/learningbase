...
class ACTConfig(PreTrainedConfig):
    ...
    def __post_init__(self):
        ...
        if self.ee_pose_loss_weight < 0.0 or self.fk_loss_weight < 0.0:
            raise ValueError("EE pose and FK loss weights must be non-negative.")
        if self.ee_position_scale_m <= 0.0:
            raise ValueError("ee_position_scale_m must be positive.")
        if self.ee_rotation_loss_weight < 0.0:
            raise ValueError("ee_rotation_loss_weight must be non-negative.")
        if (self.ee_pose_loss_weight > 0.0 or self.fk_loss_weight > 0.0) and not self.kinematics_urdf_path:
            raise ValueError("kinematics_urdf_path is required when an EE auxiliary loss is enabled.")
        if self.kinematics_urdf_sha256 is not None and len(self.kinematics_urdf_sha256) != 64:
            raise ValueError("kinematics_urdf_sha256 must be a 64-character SHA256 digest.")
        if (self.ee_pose_loss_weight > 0.0 or self.fk_loss_weight > 0.0) and self.normalization_mapping.get(
            "ACTION"
        ) is not NormalizationMode.MEAN_STD:
            raise ValueError("W1 EE auxiliary losses currently require ACTION normalization mode MEAN_STD.")
