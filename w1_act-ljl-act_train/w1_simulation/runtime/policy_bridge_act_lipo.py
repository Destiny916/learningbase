...
@dataclass(frozen=True)
class RuntimeLipoConfig:
    execution_horizon: int
    replan_threshold: float
    inference_budget_ms: float
    lipo_blend_policy_points: int
    replan_margin_policy_points: int
    policy_hz: float
    sample_factor: int
    ...
    def __post_init__(self) -> None:
        ...
        if (
            isinstance(self.replan_threshold, bool)
            or not isinstance(self.replan_threshold, int | float)
            or not math.isfinite(self.replan_threshold)
            or not 0.0 < self.replan_threshold < 1.0
        ):
            raise ValueError("replan_threshold must be finite and in (0, 1)")
        if (
            isinstance(self.inference_budget_ms, bool)
            or not isinstance(self.inference_budget_ms, int | float)
            or not math.isfinite(self.inference_budget_ms)
            or self.inference_budget_ms < 0.0
        ):
        ...
        if isinstance(self.sample_factor, bool) or not isinstance(self.sample_factor, int):
    ...
    def trigger_control_points(self) -> int:
        return scaled_control_points(self.trigger_policy_points, self.sample_factor)

    @property
    def lipo_blend_control_points(self) -> int:
        return scaled_control_points(self.lipo_blend_policy_points, self.sample_factor)
...
class LipoPolicyBridgeNode(blocking.W1PolicyBridgeBase):
    def __init__(self) -> None:
        ...
        self.declare_parameter("replan_threshold", float(DEFAULT_PROFILE.runtime["replan_threshold"]))
        self.declare_parameter("inference_budget_ms", float(DEFAULT_PROFILE.runtime["inference_budget_ms"]))
        self.declare_parameter("lipo_blend_points", int(DEFAULT_PROFILE.runtime["lipo_blend_policy_points"]))
        self.declare_parameter(
            "replan_margin_policy_points",
            int(DEFAULT_PROFILE.runtime["replan_margin_policy_points"]),
        )
        self.lipo_config = RuntimeLipoConfig(
            execution_horizon=blocking.CHUNK_SIZE,
            replan_threshold=self.get_parameter("replan_threshold").value,
            inference_budget_ms=self.get_parameter("inference_budget_ms").value,
            lipo_blend_policy_points=self.get_parameter("lipo_blend_points").value,
            replan_margin_policy_points=self.get_parameter("replan_margin_policy_points").value,
            policy_hz=self.policy_hz,
            sample_factor=self.sample_factor,
        )
        self.lipo_trigger_policy_points = self.lipo_config.trigger_policy_points
        self.lipo_blend_policy_points = self.lipo_config.lipo_blend_policy_points
        self.lipo_trigger_control_points = self.lipo_config.trigger_control_points
        self.lipo_blend_control_points = self.lipo_config.lipo_blend_control_points
        ...
        self.get_logger().info(
            ...
            f"threshold={self.lipo_config.replan_threshold:.3f} "
            f"trigger={self.lipo_trigger_policy_points}policy_points/"
            f"{self.lipo_trigger_control_points}control_points "
            f"blend={self.lipo_blend_policy_points}policy_points/"
            f"{self.lipo_blend_control_points}control_points "
            f"budget={self.lipo_config.inference_budget_ms:.1f}ms "
